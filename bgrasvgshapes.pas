...
uses
  Classes, SysUtils, BGRAUnits, BGRATypes, {$IFNDEF FPC}{msxmldom,  msxml,} xmldom, XMLIntf, msxmldom, XMLDoc,{$ELSE}laz2_DOM,{$ENDIF} BGRAPath, BGRABitmapTypes,
  BGRACanvas2D, BGRASVGType;

type
  {$IFNDEF FPC}
  TDOMElement  = IDOMElement; //msxml.IXMLDOMElement;
  TDOMNode     = IDOMNode; //msxml.IXMLDOMNode;
//  TXMLDocument = msxml.IXMLDOMDocument;
  ...
  TSVGStyle = class(TSVGElement)
   private
     ...
     function GetStyle(sid: integer): TSVGStyleItem;
     procedure SetStyle(sid: integer; sr: TSVGStyleItem);
  ...
  TSVGContent = class
    protected
      ...
      FDoc: TXMLDocument;
      FElements: {$IFDEF BDS}TList{$ELSE}TFPList{$ENDIF};
...
begin
  ...
  fs := [];
  if fontBold then   fs := fs +[fsBold];
  if fontItalic then fs := fs +[fsItalic];
...
function TSVGStyle.GetStyle(sid: integer): TSVGStyleItem;
begin
...
procedure TSVGStyle.SetStyle(sid: integer; sr: TSVGStyleItem);
begin
  if IsValidID(sid) then
    FStyles[sid]:= sr
  else
...
begin
  ...
  begin
    if s <> '' then s := s +' ';
    with AValue[i] do
      s := s +TCSSUnitConverter.formatValue(x)+' '+TCSSUnitConverter.formatValue(y);
  end;
  points := s;
...
var cur: TDOMNode;
    {$IFnDEF FPC}IElement : IDOMElement;{$ENDIF}
begin
  FDoc := ADocument;
  FDomElem := AElement;
  FDataLink := ADataLink;
  FElements := {$IFDEF BDS}TList{$ELSE}TFPList{$ENDIF}.Create;
  FUnits := AUnits;
  ...
  begin
    ...
    if cur is TDOMElement then
      FElements.Add(CreateSVGElementFromNode(ADocument,TDOMElement(cur),FUnits,ADataLink,ADataParent));
    {$ELSE}
    if  Supports(cur, IDOMElement, IElement) then
      FElements.Add(CreateSVGElementFromNode(ADocument, IElement,FUnits,ADataLink,ADataParent));
    {$ENDIF}
    cur := cur.nextSibling;
  end;
end;
