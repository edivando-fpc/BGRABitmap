...
var
  ...
  var
    ...
    begin
      ...
      begin
        if i > 0 then buf := buf + LineEnding;
        buf := buf +bufLines[i];
      end;

      bufLines.Free;

      case AnsiIndexStr(UpperCase(curBidi), ['CS','L','EN','ES','ET','R','AL','AN','NSM','BN','B','S','WS','ON']) of
      00: WriteLn(tOut,buf+': result := ubcCommonSeparator;');
      01: WriteLn(tOut,buf+': result := ubcLeftToRight;');
      02: WriteLn(tOut,buf+': result := ubcEuropeanNumber;');
      03: WriteLn(tOut,buf+': result := ubcEuropeanNumberSeparator;');
      04: WriteLn(tOut,buf+': result := ubcEuropeanNumberTerminator;');
      05: WriteLn(tOut,buf+': result := ubcRightToLeft;');
      06: WriteLn(tOut,buf+': result := ubcArabicLetter;');
      07: WriteLn(tOut,buf+': result := ubcArabicNumber;');
      08: WriteLn(tOut,buf+': result := ubcNonSpacingMark;');
      09: WriteLn(tOut,buf+': result := ubcBoundaryNeutral;');
      10: WriteLn(tOut,buf+': result := ubcParagraphSeparator;');
      11: WriteLn(tOut,buf+': result := ubcSegmentSeparator;');
      12: WriteLn(tOut,buf+': result := ubcWhiteSpace;');
      13: WriteLn(tOut,buf+': result := ubcOtherNeutrals;');
      end;
      codeCount:= 0;
    end;
...
var
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                bracketStackPos := bracketStackPos +1;
              end else
                break;
            end else
            begin
  ...
  begin
    ...
    UNICODE_LEFT_TO_RIGHT_OVERRIDE,UNICODE_LEFT_TO_RIGHT_EMBEDDING:
      if odd(minBidiLevel) then minBidiLevel := minBidiLevel + 1;
    UNICODE_RIGHT_TO_LEFT_OVERRIDE,UNICODE_RIGHT_TO_LEFT_EMBEDDING:
      if not odd(minBidiLevel) then minBidiLevel :=  minBidiLevel +1;
    end;
  ...
  begin
    ...
    begin
      ...
      ubcRightToLeft,ubcArabicLetter:
        if not Odd(result[curIndex].bidiLevel) then result[curIndex].bidiLevel := result[curIndex].bidiLevel +1;
      ubcEuropeanNumber,ubcArabicNumber:
        if Odd(result[curIndex].bidiLevel) then result[curIndex].bidiLevel := result[curIndex].bidiLevel +1
        else result[curIndex].bidiLevel := result[curIndex].bidiLevel +2;
      ubcLeftToRight: if Odd(result[curIndex].bidiLevel) then result[curIndex].bidiLevel := result[curIndex].bidiLevel +1;
      end;
      curIndex := a[curIndex].nextInIsolate;
    end;
  end;
  ...
  begin
    ...
    UNICODE_LEFT_TO_RIGHT_ISOLATE: if Odd(minBidiLevel) then minBidiLevel := minBidiLevel +1;
    UNICODE_RIGHT_TO_LEFT_ISOLATE: if not Odd(minBidiLevel) then minBidiLevel := minBidiLevel +1;
    else
