...
uses
  Classes, SysUtils, BGRATypes,{$IFNDEF FPC}Types, GraphType, BGRAGraphics, xmldom, XMLIntf, msxmldom, XMLDoc,{$ELSE}laz2_DOM, {$ENDIF}
  BGRABitmapTypes, BGRAUnits, BGRASVGShapes, BGRACanvas2D, BGRASVGType;

{$IFNDEF FPC}
type
  TCSSUnit = BGRAUnits.TCSSUnit;
  TXMLDoc =  XMLDoc.TXMLDocument;
{$ENDIF}
...
uses {$IFDEF FPC}laz2_XMLRead, laz2_XMLWrite, {$ENDIF} BGRAUTF8;
...
begin
  ...
  begin
    ...
    case par.HorizAlign of
      taCenter: result.Left := result.Left +((w0-w)/2);
      taRightJustify: result.Left := result.Left +(w0-w);
    end;
    case par.VertAlign of
      tlCenter: result.Top := result.Top +((h0-h)/2);
      tlBottom: result.Top := result.Top +(h0-h);
    end;
  end;
...
begin
  ...
  {$ELSE}
  result := TSVGPreserveAspectRatio.Parse(AttributeDef['preserveAspectRatio','xMidYMid']);
  {$ENDIF}
end;
...
begin
  ...
  begin
    ...
    FXml.Active := True;
    {$ENDIF}
    FRoot := FXml.{$IFNDEF FPC}DOMDocument.{$ENDIF}CreateElement('svg');
    FUnits := TSVGUnits.Create(FRoot,@FDefaultDpi);
    FUnits.OnRecompute:= {$IFDEF OBJ}@{$ENDIF}UnitsRecompute;
    FDataLink := TSVGDataLink.Create;
    FContent := TSVGContent.Create(FXml,FRoot,FUnits,FDataLink,nil);
    FXml.{$IFNDEF FPC}DOMDocument.{$ENDIF}AppendChild(FRoot);
  end;
end;
...
begin
  ...
  root := xml.{$IFNDEF FPC}DOMDocument.{$ENDIF}FirstChild;
  {$IFDEF FPC}
  while (root <> nil) and not (root is TDOMElement) do root := root.NextSibling;
  {$ELSE}
  while (root <> nil) and not Supports(root, TDOMElement) do root := root.NextSibling;
  {$ENDIF}
  if root = nil then
  begin
    {$IFDEF FPC}
...
begin
  ...
  case AHorizAlign of
    taCenter: sx := sx +((w - sw)/2);
    taRightJustify: sx := sx +(w - sw);
  end;
  case AVertAlign of
    tlCenter: sy := sy +((h - sh)/2);
    tlBottom: sy := sy +(h - sh);
  end;
