...
uses
  Types, SysUtils, BGRATypes,{$IFNDEF FPC}GraphType, {$ENDIF} BGRAGraphics, BGRABitmapTypes;
...
type
  ...
  TCubicKernel = class(TWideKernelFilter)
    function pow3(x: single): single; {$ifdef inline}inline;{$endif}
    function Interpolation(t: single): single; override;
    function ShouldCheckRange: boolean; override;
    function KernelWidth: single; override;
  end;
...
var
  x_src,y_src, y_src2, prev_y_src, prev_y_src2: BGRANativeInt;
  inc_x_src, mod_x_src, acc_x_src, inc_y_src, mod_y_src, acc_y_src,
  acc_x_src2, acc_y_src2: BGRANativeInt;
  x_dest, y_dest: BGRANativeInt;

  PDest, PSrc1, PSrc2: PBGRAPixel;
  vertColors: packed array[1..2] of TBGRAPixel;
  DeltaSrcX: BGRANativeInt;
  targetRect: TRect;
  tempData: PBGRAPixel;
...
var xb,yb,ys: BGRANativeInt;
    pdest: PBGRAPixel;
    psrc1,psrc2: PBGRAPixel;
    asum,maxsum: BGRANativeUInt;
    newWidth,newHeight: BGRANativeInt;
    r,g,b: BGRANativeUInt;
begin
  ...
  ys := 0;
  maxsum := 255*BGRANativeInt(factorX)*BGRANativeInt(factorY);
...
procedure DownSamplePutImage(source: TBGRACustomBitmap; factorX, factorY: integer;
  dest: TBGRACustomBitmap; OffsetX, OffsetY: Integer; ADrawMode: TDrawMode);
var xb,yb,ys,iy,ix: BGRANativeInt;
    pdest,psrci: PBGRAPixel;
    psrc: array of PBGRAPixel;
    asum,maxsum: BGRANativeUInt;
    newWidth,newHeight: BGRANativeInt;
    r,g,b,nbi: BGRANativeUInt;
begin
...
var
  ...
  ixsrc1, ixsrc2, iysrc1, iysrc2, ixsrc1p1, ixsrc2m1, iysrc1p1, iysrc2m1: int32or64; //integer;
    ...
    begin
      ...
      begin
        ...
        begin
          cBorder := psrc^;
          Inc(psrc);
          rSum := rSum +(cBorder.red * cBorder.alpha   * factVert1);
          gSum := gSum +(cBorder.green * cBorder.alpha * factVert1);
          bSum := bSum +(cBorder.blue * cBorder.alpha  * factVert1);
          aSum := aSum +(cBorder.alpha * factVert1);
          Sum  := Sum  +factVert1;
        end;
        ...
        begin
          ...
          begin
            cBorder := psrc^;
            Inc(psrc);
            rSum := rSum +(cBorder.red * cBorder.alpha   * factVert2);
            gSum := gSum +(cBorder.green * cBorder.alpha * factVert2);
            bSum := bSum +(cBorder.blue * cBorder.alpha  * factVert2);
            aSum := aSum +(cBorder.alpha * factVert2);
            Sum  := Sum  +factVert2;
          end;
        end;
      end;
      ...
      begin
        ...
        begin
          cBorder := psrc^;
          Inc(psrc, lineDelta);
          rSum := rSum +(cBorder.red * cBorder.alpha   * factHoriz1);
          gSum := gSum +(cBorder.green * cBorder.alpha * factHoriz1);
          bSum := bSum +(cBorder.blue * cBorder.alpha  * factHoriz1);
          aSum := aSum +(cBorder.alpha * factHoriz1);
          Sum  := Sum  +factHoriz1;
        end;
        ...
        begin
          ...
          begin
            cBorder := psrc^;
            Inc(psrc, lineDelta);
            rSum := rSum +(cBorder.red * cBorder.alpha   * factHoriz2);
            gSum := gSum +(cBorder.green * cBorder.alpha * factHoriz2);
            bSum := bSum +(cBorder.blue * cBorder.alpha  * factHoriz2);
            aSum := aSum +(cBorder.alpha * factHoriz2);
            Sum  := Sum  +factHoriz2;
          end;
        end;
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            cFull := psrc^;
            rSum  := rSum  +(cFull.red   * cFull.alpha);
            gSum  := gSum  +(cFull.green * cFull.alpha);
            bSum  := bSum  +(cFull.blue  * cFull.alpha);
            aSum  := aSum  +cFull.alpha;
            Sum   := Sum   +1;
            Inc(psrc);
          end;
          Inc(psrc, delta);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          sum.sumA := sum.sumA +(sumA*w);
          sum.sumR := sum.sumR +(sumR*w);
          sum.sumG := sum.sumG +(sumG*w);
          sum.sumB := sum.sumB +(sumB*w);
        end;
      end;
