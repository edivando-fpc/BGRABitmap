...
uses
  Classes, SysUtils, BGRATypes, BGRATransform, BGRABitmapTypes, BGRAUnits,
  {$IFNDEF FPC}xmldom, XMLIntf, msxmldom, XMLDoc,{$ELSE}laz2_DOM, {$ENDIF} BGRACanvas2D,
  {$IFDEF OBJ}fgl{$ELSE}generics.collections{$ENDIF}, BGRAGraphics;

type
  {$IFNDEF FPC}
  TDOMElement  =  IDOMElement;//;msxml.IXMLDOMElement;
//  TXMLDocument = msxml.IXMLDOMDocument;
  {$ENDIF}

  ArrayOfFloat = array of single;

  TSVGElement = class;
  TSVGElementList = {$IFDEF OBJ}specialize TFPGList{$ELSE}TList{$ENDIF}<TSVGElement>;
  ...
  TSVGElement = class
    private
      ...
      function GetStyle(AName,ADefault: string): string; overload;
      function GetStyle(AName: string): string; overload;
  ...
  public
    ...
    function ParseSymbol: {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF};
    function ParseTransform: TAffineMatrix;
    procedure SkipSymbol(ASymbol: {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF});
    procedure SkipUpToSymbol(ASymbol:{$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF});
...
begin
  ...
  begin
    result := '';
    case HorizAlign of
    taCenter: result := result +'xMid';
    taRightJustify: result := result +'xMax';
    else result := result +'xMin';
    end;
    case VertAlign of
    tlCenter: result := result +'YMid';
    tlBottom: result := result +'YMax';
    else result := result +'YMin';
    end;
    if Slice then result := result +' slice' else result := result +' meet';
  end;
end;
...
begin
  ...
  repeat
    ...
    begin
      case AnsiIndexStr(copy(id,2,3), ['Min','Mid','Max']) of
      00: result.HorizAlign := taLeftJustify;
      01: result.HorizAlign := taCenter;
      02: result.HorizAlign := taRightJustify;
      end;
      case AnsiIndexStr(copy(id,6,3), ['Min','Mid','Max']) of
      00: result.VertAlign := tlTop;
      01: result.VertAlign := tlCenter;
      02: result.VertAlign := tlBottom;
      end;
    end;
  until id = '';
  p.Free;
...
function TSVGParser.ParseSymbol: {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF};
begin
  while (FPos <= length(FText)) and (FText[FPos] in[#0..#32,',']) do inc(FPos);
  if (FPos <= length(FText)) and not (FText[FPos] in['A'..'Z','a'..'z','0'..'9']) then
  begin
    result := {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF}(FText[FPos]);
    inc(FPos);
  end else
    result := #0;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      SkipSymbol(',');
      ty := ParseFloat;
      result := AffMatrixMult(result, AffineMatrixTranslation(tx,ty));
    end else
    ...
    begin
      ...
      result := AffMatrixMult(result, AffineMatrixScale(tx,ty));
    end else
    ...
    begin
      ...
      SkipSymbol(',');
      ty := ParseFloat;
      result := AffMatrixMult(AffMatrixMult(AffMatrixMult(result, AffineMatrixTranslation(tx,ty)),AffineMatrixRotationDeg(angle)),
                AffineMatrixTranslation(-tx,-ty));
    end else
    if compareText(kind,'skewx')=0 then
    begin
      angle := ParseFloat;
      result := AffMatrixMult(result,AffineMatrixSkewXDeg(angle));
    end else
    if compareText(kind,'skewy')=0 then
    begin
      angle := ParseFloat;
      result := AffMatrixMult(result,AffineMatrixSkewYDeg(angle));
    end;
    SkipUpToSymbol(')');
  end;
end;

procedure TSVGParser.SkipSymbol(ASymbol: {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF});
begin
  while (FPos <= length(FText)) and (FText[FPos] in[#0..#32,',']) do inc(FPos);
  if (FPos <= length(FText)) and ({$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF}(FText[FPos]) = ASymbol) then inc(FPos);
end;

procedure TSVGParser.SkipUpToSymbol(ASymbol: {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF});
begin
  while (FPos <= length(FText)) and ({$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF}(FText[FPos])<>ASymbol) do inc(FPos);
  if (FPos <= length(FText)) and ({$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF}(FText[FPos])=ASymbol) then inc(FPos);
end;
...
begin
  ...
  repeat
    ...
    if ((result = '') or (result = 'inherit')) and ACanInherit and
      ...
      {$ELSE}
       Supports(curNode.ParentNode, TDOMElement) then
      {$ENDIF}
...
begin
  ...
  if result.CSSUnit <> cuCustom then
    if units.DpiScaleY = 0 then
      ...
      result.value := Result.Value / Units.DpiScaleY;
end;
  ...
  begin
    if result.CSSUnit <> cuCustom then
      if units.DpiScaleX = 0 then
        ...
        result.value := result.value / Units.DpiScaleX;
  end;
end;
...
function TSVGElement.GetStyle(AName,ADefault: string): string;
...
begin
  ...
  {$ELSE}
    result:= GetInternal( AttributeDef['style',s] );
  {$ENDIF}
end;

function TSVGElement.GetStyle(AName: string): string;
begin
...
begin
  ...
  begin
    if s <> '' then s := s +' ';
    s := s +TCSSUnitConverter.formatValue(AValue[i])+' ';
  end;
  strokeDashArray := s;
end;
...
begin
  ...
  begin
    ...
    begin
      if ruleset[length(ruleset)] <> ';' then ruleset := ruleset +'; ';
    end;
    ruleset := ruleset +AName+': '+AValue;
  end;
...
begin
  ...
  {$ELSE}
  FDomElem := ADocument.DOMDocument.createElement(ATag);
  {$ENDIF}
  FUnits := AUnits;
end;
...
begin
  ...
  {$ELSE}
  result := Assigned(FDomElem.attributes) and Assigned(FDomElem.attributes.GetNamedItem(AName));
  {$ENDIF}
end;
