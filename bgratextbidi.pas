...
uses
  Classes, SysUtils, BGRATypes, BGRAGraphics, BGRABitmapTypes, BGRAUTF8, BGRAUnicode, BGRATransform;
...
type
  ...
  private
    ...
    function GetUnicodeChar(APosition0: integer): BGRACardinal;
  ...
  public
    ...
    property UnicodeChar[APosition0: integer]: BGRACardinal read GetUnicodeChar;
...
begin
  ...
  result := AffMatrixMult(Matrix,TAffineBox.AffineBox(FBrokenLine[AIndex].rectF));
end;
...
begin
  ...
  if (AIndex < 0) or (AIndex >= FParagraphCount) then
    raise ERangeError.Create('Invalid index');
  result := AffMatrixMult(Matrix,TAffineBox.AffineBox(FParagraph[AIndex].rectF));
end;
...
begin
  ...
  if (AIndex < 0) or (AIndex >= FPartCount) then
    raise ERangeError.Create('Invalid index');
  result := AffMatrixMult(Matrix,TAffineBox.AffineBox(FPart[AIndex].rectF));
end;
...
function TBidiTextLayout.GetUnicodeChar(APosition0: integer): BGRACardinal;
var p : PChar; //@{$IFDEF FPC}PChar{$ELSE}PAnsiChar{$ENDIF};
  charLen: Integer;
begin
...
begin
  FMatrix := AffMatrixMult(AffineMatrixTranslation(FTopLeft.x, FTopLeft.y),AffineMatrixRotationDeg(-GetFontOrientation));
...
begin
  ...
  begin
    ...
    begin
      ...
      else result.x := result.x -((result.x+extraW - combW) * 0.5);
    end;
  end;
end;
...
var
  p: PChar; //@{$IFDEF FPC}PChar{$ELSE}PAnsiChar{$ENDIF};
  pEnd: Pointer;
...
begin
  ...
  begin
    ...
    begin
      if FBidi[i].BidiInfo.IsEndOfParagraph then FParagraphCount := FParagraphCount +1;
      FUnbrokenLineCount := FUnbrokenLineCount +1;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FBrokenLineCount := FBrokenLineCount + 1;
...
begin
  ...
  for i := 0 to FPartCount-1 do
    with (AffMatrixMult(Matrix,(FPart[i].rectF.TopLeft + FPart[i].posCorrection))) do
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          result.Top := AffMatrixMult(Matrix,result.Top);
          result.Bottom := AffMatrixMult(Matrix,result.Bottom);
  ...
  begin
    result.Top := FTopLeft;
    result.Bottom := AffMatrixMult(FMatrix,PointF(0,FLineHeight));
    result.RightToLeft := false;
    result.PreviousTop := EmptyPointF;
    result.PreviousBottom := EmptyPointF;
    result.PreviousRightToLeft := false;
...
begin
  NeedLayout;
  untransformedPos := AffMatrixMult(FMatrixInverse,APosition);
...
begin
  ...
  begin
    ...
    if endCaret.PartIndex <> -1 then endCaret.PartIndex := endCaret.PartIndex -1;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          result := ConcatPointsF([result, AffMatrixMult(Matrix,TAffineBox.AffineBox(r).AsPolygon), PointsF([EmptyPointF])]);
  ...
  if APixelCenteredCoordinates then
    for i := 0 to high(result) do
      if not isEmptyPointF(result[i]) then result[i] := result[i] +PointF(0.5,0.5);
end;
...
begin
  ...
  begin
    ...
    result.PreviousTop := AffMatrixMult(Matrix,result.PreviousTop);
    result.PreviousBottom := AffMatrixMult(Matrix,result.PreviousBottom);
