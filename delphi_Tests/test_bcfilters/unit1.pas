...
uses
  ...
  BGRABitmapTypes, BCTypes, BGRATextFX, Math, BCEffect, BCBaseCtrls, Vcl.Mask,
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure fs2Change(Sender: TObject);
  private
...
var
  sTemp : string;
begin
  sTemp := ExtractFilePath(Application.ExeName) + '\'  + 'Penguins.jpg';
  Image := TBGRABitmap.Create(sTemp);
...
procedure TForm1.fs2Change(Sender: TObject);
begin
  vs1.RedrawBitmap;
end;
...
begin
  ...
  { Apply filter 1 }
  SimpleFilter(Bitmap, StrToTBCSimpleFilter(ListBox1.Items[ListBox1.ItemIndex]));
  { Apply filter 2 }
  SimpleFilter(Bitmap, StrToTBCSimpleFilter(ListBox2.Items[ListBox2.ItemIndex]));
  { Apply filter 3 }
  SimpleFilter(Bitmap, StrToTBCSimpleFilter(ListBox3.Items[ListBox3.ItemIndex]));
  { Apply filter 4 }
  SimpleFilter(Bitmap, StrToTBCSimpleFilter(ListBox4.Items[ListBox4.ItemIndex]));
