...
uses
  Controls, Graphics, BCButton, BCTypes, BCBaseCtrls,BGRABitmap, BGRABitmapTypes;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if TBCButton(WinControl.Controls[i]).Caption = '-' then
        begin
          StyleRibbonButtonsSample(WinControl.Controls[i], rsSeparator);
          TBCButton(WinControl.Controls[i]).Caption := '';
        end
        else
...
begin
  with ABitmap do
  begin
    DrawVertLine(10, 0, 300, ColorToBGRA(clBlack));  //Right
    DrawHorizLine(10, 20, 400, ColorToBGRA(MainColor)); // Bottom
//    DrawVertLine(0, 0, Height-1, ColorToBGRA(MainColor)); //Left
//    DrawHorizLine(1, 0, Width - 2, $00E6E6E6); // Top
//    GradientFill(0, 0, Width, Height, BGRA(197, 80, 223, 120),
//      BGRA(220, 229, 242, 255), gtRadial, PointF(50, 120), PointF(0, Height), dmSet);
  end;
end;
