...
uses
  Windows, classes, types, BGRABitmap, BGRABitmapTypes, SysUtils, Forms, Controls,
  Graphics, StdCtrls, ExtCtrls, Menus, Dialogs, ActnList,
  Vcl.ImgList, Vcl.Buttons,
  BGRAGraphics, FPImage, BGRATypes, BGRAMultiFileType, GraphType, BGRAVirtualScreen,
  BCButton, BCPanel, BGRAImageList, BGRAResizeSpeedButton, uRibbon,
  BGRASpeedButton, BCBaseCtrls;
...
//  Width := StrToInt(XMLConfigMain.GetValue('FormWidth', '600'));
  ...
  TabColor := 'blue'; //XMLConfigMain.GetValue('Color', 'blue');
...
begin
  ...
  //end;
  uRibbon.RibbonMenu(btnFileMenu);
...
begin
  ...
  begin
    ...
    if mnu.Name =
      'mnuColorChange' then  TabColor := 'blue';
    if mnu.Name =
      'mnuColorGreen' then  TabColor := 'green';
    if mnu.Name =
      'mnuColorRed' then  TabColor := 'red';
    if mnu.Name =
      'mnuColorOrange' then  TabColor := 'orange';
    if mnu.Name =
      'mnuColorLiteBlue'then  TabColor := 'liteblue';
    if mnu.Name =
      'mnuColorPetrol' then  TabColor := 'petrol';
    if mnu.Name =
      'mnuColorPurple' then  TabColor := 'purple';
    ColorChange(self);
  end;
...
the time (so it's visible in tray) }
  ...
  {$else}
  // Something to be done for OS <> Windows?
  if MessageDlg('Warning' + #10#13 + 'Hide application? It wont be shown on launch pad.',
...
var
  i: integer; //  vsBody.Top := 66 // needs to be replaced by variable
  step: integer;
begin
  i := 66;
  step := 10;
...
//    frmMain.DisableAutoSizing; //reduce flicker
    ...
    if tab.Name =  'btnTabEdit' then
     ...
     if tab.Name = 'btnTabSettings' then
      begin
        btnTabHome.Down := False;
        btnTabEdit.Down := False;
