...
var
  n:      integer;
  PChangePix, PNewPix: {$IFDEF BDS}PBGRAPixel;{$ELSE}PBGRALongWord;{$ENDIF}
  newpix: BGRALongWord;
  MemPixEraseColor: BGRALongWord;
  {$IFDEF BDS}Temp : TBGRAPixel;{$ENDIF}
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          {$IFDEF BDS}
          move(MemPixEraseColor , PChangePix^, SizeOf(BGRADWord));
          move(newpix , temp, SizeOf(BGRADWord));
          DrawPixelInlineNoAlphaCheck(PChangePix, temp);
          {$ELSE}
          ...
          {$IFDEF BDS}move(MemPixEraseColor, PChangePix^, sizeof(BGRADWord));{$ELSE}PChangePix^ := MemPixEraseColor;{$ENDIF} //efface précédent
{           if newpix and AlphaMask > AlphaLimit then PChangePix^ := newpix or AlphaMask //pixel opaque
