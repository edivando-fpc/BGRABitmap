...
uses
  Types, Classes, SysUtils, Graphics, BGRATypes, {$IFNDEF FPC}GraphType, BGRAGraphics,{$ENDIF}
  BGRABitmapTypes, BGRATypewriter, BGRATransform, BGRACanvas2D, BGRAText;
...
var
  ...
  polygonF: {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
...
//      shouldRemove := false;
  ...
  procedure AddPolygon(n: integer);
    ...
    var ptsF: {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
        nbPtsF: integer;
...
begin
  ...
  factor := zoom/iUnit;
  offset := zoom*0.5;
  if PixelCenteredCoordinates then Offset := Offset -0.5;
...
begin
  ...
  c2D.fillStyle(c);
  ofs := PointF(x,y);
  ofs := ofs +AffMatrixMult(AffineMatrixRotationDeg(-orientation*0.1),PointF(0,FVectorizedFont.FullHeight*0.5));
...
begin
  ...
  ofs := PointF(x,y);
  ofs := ofs +PointF(0,FVectorizedFont.FullHeight*0.5);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        distance := 0;
        if (fsBold in FDirectoryContent[i].FontStyle) xor (fsBold in FStyle) then distance := distance +10;
        if (fsItalic in FDirectoryContent[i].FontStyle) xor (fsItalic in FStyle) then distance := distance +5;
        if (fsStrikeOut in FDirectoryContent[i].FontStyle) xor (fsStrikeOut in FStyle) then distance := distance +1;
        if (fsUnderline in FDirectoryContent[i].FontStyle) xor (fsUnderline in FStyle) then distance := distance +1;
...
begin
  TypeWriterMatrix := AffMatrixMult(AffMatrixMult(AffMatrixMult(FFontMatrix,AffineMatrixRotationDeg(-Orientation*0.1)),AffineMatrixScale(FFullHeight,FFullHeight)),AffineMatrixLinear(PointF(1,0),PointF(-FItalicSlope,1)));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if g <> nil then
      begin
        result.x := result.x +g.Width*FullHeight;
...
begin
  ...
  begin
    ...
    begin
      ...
      if g <> nil then
      begin
        totalWidth := totalWidth +g.Width*FullHeight;
  ...
  begin
    ...
    begin
      ...
      for i := 0 to high(underlinePoly) do
        underlinePoly[i] := AffMatrixMult(m,underlinePoly[i]);
...
begin
  ...
  begin
    ...
    begin
      m := GetTextMatrix(ATextUTF8, X,Y,AAlign);
      ADest.moveTo(AffMatrixMult(m,underlinePoly[0]));
      for i := 1 to high(underlinePoly) do
        ADest.lineTo(AffMatrixMult(m,underlinePoly[i]));
      ADest.closePath;
    end;
  end;
  if StrikeOutDecoration and (Resolution > 0) then
...
begin
  ...
  ItalicSlope := 0;
  step := AffMatrixMult(TypeWriterMatrix,PointF(0,1));
  ...
  begin
    ...
    X := X +(step.X*lineShift);
    Y := Y +(step.Y*lineShift);
    repeat
  ...
  begin
    ...
    case lineAlignment of
    twaMiddle: ;
    twaBottomLeft,twaBottomRight: lineShift := lineShift -0.5;
    twaTopRight,twaTopLeft : lineShift := lineShift +0.5;
    end;
...
begin
  ...
  if (length(FDirectory) > 0) and not (FDirectory[length(FDirectory)] in AllowDirectorySeparators) then
    FDirectory := FDirectory +DirectorySeparator;
...
begin
  ...
  {$ELSE}
  LEReadVar(AStream, result.Style);
  {$ENDIF}
