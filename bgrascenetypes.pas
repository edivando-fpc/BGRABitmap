...
uses {$IFNDEF FPC}Types, GraphType,{$ENDIF} BGRATypes, BGRABitmapTypes, BGRASSE, BGRAMatrix3D, BGRAColorInt;
...
type
  ...
  IBGRALight3D = interface
    ['{85C683B6-07AC-4B8D-9324-06BC22882433}']
  ...
  IBGRAPointLight3D = interface(IBGRALight3D)
    ['{C939900D-DDD6-49F0-B1E9-E29F94FDB4C8}']
  ...
  IBGRADirectionalLight3D = interface(IBGRALight3D)
    ['{8D575CEE-8DD2-46FB-9BCC-17DE3DAAF53D}']
  ...
  IBGRAMaterial3D = interface
    ['{35B5BBE7-D3BD-43A3-AAA0-A40AC61B231B}']
  ...
  IBGRANormal3D = interface
    ['{B87D4118-6D95-4030-9BCA-FACB6F2A9F84}']
  ...
  IBGRAVertex3D = interface
    ['{83E024DF-CF50-47B4-BBBA-A9CDBEF04A5D}']
    function GetColor: TBGRAPixel;
    function GetCustomFlags: BGRADWord;
    function GetCustomNormal: TPoint3D;
    function GetCustomNormal_128: TPoint3D_128;
    ...
    procedure SetColor(const AValue: TBGRAPixel);
    procedure SetCustomFlags(AValue: BGRADWord);
    procedure SetCustomNormal(AValue: TPoint3D);
    procedure SetCustomNormal_128(AValue: TPoint3D_128);
    ...
    property CustomFlags: BGRADWord read GetCustomFlags write SetCustomFlags;
    function GetAsObject: TObject;
  end;
  ...
  IBGRAPart3D = interface
    ['{8377F5D3-4F7D-4249-ACFC-5C8032F856A2}']
  ...
  IBGRAFace3D = interface
    ['{E0141BB8-95A6-4444-8A33-9B564D154E26}']
    procedure FlipFace;
  ...
  IBGRAObject3D = interface
    ['{6B4CE4B3-5713-44B4-859D-CA0DBF33C521}']
    procedure Clear;
    function GetColor: TBGRAPixel;
    ...
    procedure SetTexture(const AValue: IBGRAScanner);
    procedure ComputeWithMatrix({$IFDEF OBJ}constref{$ELSE}const{$ENDIF} AMatrix: TMatrix3D; {$IFDEF OBJ}constref{$ELSE}const{$ENDIF} AProjection: TProjection3D);
    procedure RemoveUnusedVertices;
...
{$IFDEF FPC}{$PUSH}{$ENDIF}{$OPTIMIZATION OFF} // avoids internal error 2012090607
...
var
  ...
  PowerTableIPos,i: BGRANativeInt;
begin
...
begin
  ...
  begin
    if ALightLightness <> 32768 then
      Context^.lightness := Context^.lightness +(CombineLightness(DiffuseLightnessTerm32768,ALightLightness))
    else
      Context^.lightness := Context^.lightness + DiffuseLightnessTerm32768;
  end else
  begin
    if FDiffuseLightness <> 32768 then
      Context^.lightness := Context^.lightness + CombineLightness(DiffuseLightnessTerm32768,CombineLightness(FDiffuseLightness,ALightLightness))
    else
      Context^.lightness := Context^.lightness + CombineLightness(DiffuseLightnessTerm32768,ALightLightness);
  end;
end;
