...
begin
  ...
  FTree.OnObjectCompare := {$IFDEF OBJ}@{$ENDIF}OnCompareItems;
end;
...
var gray1, gray2: BGRANativeUInt;
  c1, c2: TBGRAPixel;
begin
  c1 := PBGRAPixel(Data1)^;
  c2 := PBGRAPixel(Data2)^;
...
begin
  for i := FLastAddedColorCount-1 downto 0 do
    if PBGRADWord(FLastAddedColors[i])^ = BGRADWord(AValue) then
    begin
...
begin
  ...
  {$ELSE}//#
  GetMem(BGRAPixel, sizeOf(TBGRAPixel));
  result := PBGRAPixel(BGRAPixel);
  {$ENDIF}
  result^ := AColor;
end;
...
var buf: {$IFDEF FPC}string;{$ELSE}Ansistring;{$ENDIF}//#
  oldPos: BGRAInt64;
  i: Integer;
begin
  result := palUnknown;
