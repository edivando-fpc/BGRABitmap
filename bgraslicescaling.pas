...
uses
  Classes, SysUtils, Types, BGRATypes, {$IFNDEF FPC}GraphType, {$ENDIF} BGRAGraphics, BGRABitmap, BGRABitmapTypes, IniFiles;
...
begin
  ...
  for p := low(TSliceRepeatPosition) to high(TSliceRepeatPosition) do
    ...
    begin
      ...
      result := result +SliceRepeatPositionStr[p];
    end;
end;
...
begin
  ...
  begin
    ...
    begin
      if not BGRAPixelEqual(p^, {<>} c0) then
      begin
        isRepeating := False;
        break;
      end;
  ...
  begin
    ...
    begin
      if not BGRAPixelEqual(middleSlice.GetPixel(x, y), {<>} c0) then
      begin
        isRepeating := False;
        break;
      end;
