...
function AffMatrixMult(M,N: TAffineMatrix): TAffineMatrix; overload;
function AffMatrixEqual(M,N: TAffineMatrix): boolean;

//matrix multiplication by a vector (apply transformation to that vector)
function AffMatrixMult(M: TAffineMatrix; V: TPointF): TPointF; overload;
function AffMatrixMult(M: TAffineMatrix; A: array of TPointF): ArrayOfTPointF; overload;
function AffMatrixMult(M: TAffineMatrix; ab: TAffineBox): TAffineBox; overload;
...
function AffMatrixMult(M, N: TAffineMatrix): TAffineMatrix;
begin
...
function AffMatrixMult(M: TAffineMatrix; V: TPointF): TPointF;
begin
...
function AffMatrixMult(M: TAffineMatrix; A: array of TPointF): ArrayOfTPointF;
var
  i: BGRANativeInt;
  ofs: TPointF;
begin
  ...
  begin
    ...
    for i := 0 to high(A) do
      result[i] := AffMatrixMult(M,A[i]);
end;

function AffMatrixMult(M: TAffineMatrix; ab: TAffineBox): TAffineBox;
begin
  result.TopLeft := AffMatrixMult(M,ab.TopLeft);
  result.TopRight := AffMatrixMult(M,ab.TopRight);
  result.BottomLeft := AffMatrixMult(M,ab.BottomLeft);
end;
...
begin
  ...
  linearInverse := AffineMatrix(M[2,2]*f,-M[1,2]*f,0,
                         -M[2,1]*f,M[1,1]*f,0);
  result := AffMatrixMult(linearInverse, AffineMatrixTranslation(-M[1,3],-M[2,3]));
end;
...
begin
  ...
  begin
    ...
    if Y < 0 then Y := Y +FSource.Height;
  end;
  ...
  begin
    ...
    if FCurX < 0 then FCurX := FCurX +FSource.Width;
  end;
end;
...
begin
  ...
  FCurTexCoord := FCurTexCoord +FStep;
end;
...
begin
  FMatrix := AffMatrixMult(FMatrix, AMatrix);
end;
...
begin
  ...
  FCurX := FCurX +FMatrix[1,1];
  FCurY := FCurY +FMatrix[2,1];
...
begin
  ...
  with (AffMatrixMult(FSourceMatrix, PointF(u,v)) + FUVVector*(u*v)) do
...
begin
  ...
  begin
    PrepareScanVert0;
    FScanFunc := {$IFDEF OBJ}@{$ENDIF}ScanVert0;
  end else
  if aa = 0 then
  begin
    PrepareScanPara;
    FScanFunc := {$IFDEF OBJ}@{$ENDIF}ScanPara
  end
  else
    FScanFunc := {$IFDEF OBJ}@{$ENDIF}ScanGeneral;
end;
...
var u1,u2,v1,v2,x,y: double;
  ...
  procedure ReturnC1C2; //@{$ifdef inline}inline;{$endif}
  var c1,c2: TBGRAPixel;
  begin
    with (AffMatrixMult(FSourceMatrix, PointF(u1,v1)) + FUVVector*(u1*v1)) do
      if FTextureInterpolation then
        c1 := FSource.ScanAt(x,y)
      else
        c1 := FSource.ScanAtInteger(System.round(x),System.round(y));
    with (AffMatrixMult(FSourceMatrix, PointF(u2,v2)) + FUVVector*(u2*v2)) do
      if FTextureInterpolation then
...
begin
  ...
  ScanVertV0 := ScanVertV0 +ScanVertVStep0;
  ScanVertDenom0 := ScanVertDenom0 +ScanVertDenomStep0;
end;
...
begin
  ...
  begin
    ScanParaBB := ScanParaBB +FCoeffs[3].y;
    if ScanParaBB <> 0 then
      ScanParaBBInv := 1/ScanParaBB
    else
      ScanParaBBInv := 1;
  end;
  ScanParaCC := ScanParaCC +FCoeffs[1].y;
end;
...
begin
  Init(ASource, APoints, ATextureInterpolation);
  FSourceMatrix := AffMatrixMult(AffineMatrixTranslation(ATexCoords[0].x,ATexCoords[0].y),
...
procedure TBGRAAffineBitmapTransform.Init(ABitmap: TBGRACustomBitmap;
  ...
  AResampleFilter: TResampleFilter; AIncludeEdges: boolean);
begin
...
constructor TBGRAAffineBitmapTransform.Create(ABitmap: TBGRACustomBitmap;
  ARepeatImage: Boolean; AResampleFilter: TResampleFilter; AIncludeEdges: boolean);
begin
...
var p: PBGRAPixel;
  n: integer;
  posXPrecision, posYPrecision: BGRANativeInt;
  deltaXPrecision,deltaYPrecision: BGRANativeInt;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        inc(p);
        posXPrecision := posXPrecision +deltaXPrecision;
        posYPrecision := posYPrecision +deltaYPrecision;
      end;
    end else
    begin
     for n := count-1 downto 0 do
...
begin
  ...
  begin
    Matrix := AffMatrixMult(AffMatrixMult(AffineMatrixTranslation(-0.5,-0.5),AffineMatrixScale(FBitmap.Width,FBitmap.Height)),Matrix);
  end else
    Matrix := AffMatrixMult(AffineMatrixScale(FBitmap.Width-1,FBitmap.Height-1),Matrix);
end;
...
begin
  ...
  {$ELSE}
  FScanAtProc:= TBGRACustomBitmap(FTexture.GetInstance).ScanAt;
  {$ENDIF}
end;
...
begin
  ...
  ScanDenom := ScanDenom +w0;
  ScanNumX  := ScanNumX  +sx;
  scanNumY  := scanNumY  +shy;
end;
