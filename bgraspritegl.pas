...
uses
  Classes, SysUtils, BGRATypes, {$IFNDEF FPC}Types, GraphType, BGRAGraphics,{$ENDIF} BGRAOpenGLType,
  BGRABitmapTypes;
    ...
    begin
      ...
      if AValue >= FrameLoopEnd+0.49 then
        begin
          ...
          begin
            AValue := AValue -Trunc((AValue-(FrameLoopStart-0.5))/loopLength)*loopLength;
      ...
      if AValue < FrameLoopStart-0.49 then
        begin
          ...
          begin
            AValue := AValue +Trunc((FrameLoopEnd+0.5-AValue)/loopLength)*loopLength;
