...
{begin  //param: eax, edx, ecx  //float: eax ecx edx
  ...
  //offset +$40 $..}
uses
  {$IFNDEF FPC}Types, GraphType,{$ENDIF} BGRATypes, BGRABitmapTypes {$ifdef CPUI386}, cpu, mmx{$endif};
...
type
  TPoint3D_128 = packed record
    x,y,z,t: single;
    class operator{$IFDEF OBJ}*{$ELSE}Multiply{$ENDIF} (const v1: TPoint3D_128; const factor: single): TPoint3D_128;
    class operator{$IFDEF OBJ}+{$ELSE}Add{$ENDIF}({$IFDEF OBJ}constref{$ELSE}const{$ENDIF} v1,v2: TPoint3D_128): TPoint3D_128;
    class operator{$IFDEF OBJ}-{$ELSE}Subtract{$ENDIF}(const v1,v2: TPoint3D_128): TPoint3D_128;
    class operator{$IFDEF OBJ}-{$ELSE}Negative{$ENDIF}(const v: TPoint3D_128): TPoint3D_128;
    class operator{$IFDEF OBJ}={$ELSE}Equal{$ENDIF}(const v1,v2: TPoint3D_128): boolean;
    class operator{$IFDEF OBJ}<>{$ELSE}NotEqual{$ENDIF}(const v1,v2: TPoint3D_128): boolean;
  end;
  PPoint3D_128 = ^TPoint3D_128;

  arrayofTPoint3D_128 = array of TPoint3D_128;

  function ToPoint3D(const point3D_128: TPoint3D_128): TPoint3D;{$ifdef inline}inline;{$endif}overload;
  function Point3D_128(const point3D: TPoint3D): TPoint3D_128;{$ifdef inline}inline;{$endif}overload;
  function Point3D_128(const pointF: TPointF): TPoint3D_128;{$ifdef inline}inline;{$endif}overload;
  function Point3D_128(x,y,z: single): TPoint3D_128;{$ifdef inline}inline;{$endif}overload;
  function Point3D_128(x,y,z,t: single): TPoint3D_128;{$ifdef inline}inline;{$endif}overload;
...
var
  ...
  DotProduct3D_128 : function ({$IFDEF OBJ}constref{$ELSE}const{$ENDIF} v1,v2: TPoint3D_128): single;
...
type
  ...
  TBasicLightingContext = packed record
    ...
    {96} dummy4: single;
    {100} dummy3: LongBool;
    {104} dummy1: BGRALongWord;
    {108} dummy2: BGRALongWord;
...
function Point3D_128(const point3D: TPoint3D): TPoint3D_128;
begin
  result.x := point3D.x;
  result.y := point3D.y;
  result.z := point3D.z;
...
class operator TPoint3D_128.{$IFDEF OBJ}+{$ELSE}Add{$ENDIF}({$IFDEF OBJ}constref{$ELSE}const{$ENDIF} v1,v2: TPoint3D_128): TPoint3D_128;
...
{$ifdef BGRASSE_AVAILABLE}
procedure Add3D_AlignedSSE(var dest: TPoint3D_128; {$IFDEF OBJ}constref{$ELSE}const{$ENDIF} src: TPoint3D_128); assembler;
asm
  movaps xmm0, [dest]
  movups xmm1, [src]
  addps xmm0, xmm1
...
procedure Add3D_NoSSE(var dest: TPoint3D_128; {$IFDEF OBJ}constref{$ELSE}const{$ENDIF} src: TPoint3D_128);
{$ifdef CPUI386} assembler;
asm
  db $d9, $00 //flds [eax]
  db $d8, $02 //fadds [edx]
...
class operator TPoint3D_128.{$IFDEF OBJ}-{$ELSE}Subtract{$ENDIF}(const v1,v2: TPoint3D_128): TPoint3D_128;
{$ifdef CPUI386} assembler;
asm
  db $d9, $02 //flds [edx]
  db $d8, $28 //fsubrs [eax]
...
class operator TPoint3D_128.{$IFDEF OBJ}-{$ELSE}Negative{$ENDIF}(const v: TPoint3D_128): TPoint3D_128;
begin
  result.x := -v.x;
  result.y := -v.y;
  result.z := -v.z;
...
class operator TPoint3D_128.{$IFDEF OBJ}={$ELSE}Equal{$ENDIF}(const v1, v2: TPoint3D_128): boolean;
begin
...
class operator TPoint3D_128.{$IFDEF OBJ}*{$ELSE}Multiply{$ENDIF}(const v1: TPoint3D_128; const factor: single): TPoint3D_128;
{$ifdef CPUI386} assembler;
asm
  db $d9, $00 //flds [eax]
...
function DotProduct3D_128_NoSSE({$IFDEF OBJ}constref{$ELSE}const{$ENDIF} v1,v2: TPoint3D_128): single;
begin
...
begin
  ...
  delta := BGRAPtrUInt(FContainer) and 15;
