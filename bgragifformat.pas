...
uses
  Classes, SysUtils, BGRATypes,{$IFNDEF FPC}Types, GraphType,{$ENDIF} BGRAGraphics, BGRABitmap, BGRABitmapTypes,
  BGRAPalette;
...
type
  ...
  TGIFSignature = packed array[1..6] of AnsiChar; //'GIF87a' or 'GIF89a'

  TGIFScreenDescriptor = packed record
    Width, Height: BGRAWord;
  ...
  TGIFData = record
    ...
    LoopCount: BGRAWord;
    Images: {$IFDEF BDS}TGifSubImageArray{$ELSE}array of TGifSubImage{$ENDIF};
  end;
...
const
  ...
  GIFInterlacedStart: array[1..4] of BGRALongInt = (0, 4, 2, 1);
  GIFInterlacedStep: array[1..4] of BGRALongInt = (8, 8, 4, 2);
...
var
  strtab:   Pstrtab;
  oldcode, curcode, clearcode, endcode: BGRALongInt;
  codesize, codelen, codemask: BGRALongInt;
  stridx:   BGRALongInt;
  bitbuf, bitsinbuf: BGRALongInt;
  bytbuf:   packed array[0..255] of byte;
  bytinbuf, bytbufidx: byte;
  endofsrc: boolean;
  xcnt, ycnt, ystep, pass: BGRALongInt;
  pdest: PBGRAPixel;
  ...
  function GetNextCode: BGRALongInt;
  begin
    ...
    begin
      ...
      bitbuf := bitbuf or (BGRALongInt(byte(bytbuf[bytbufidx])) shl bitsinbuf);
      Inc(bytbufidx);
      Dec(bytinbuf);
      Inc(bitsinbuf, 8);
    end;
  ...
  procedure AddStr2Tab(prefix: Pstr; suffix: BGRALongInt);
  begin
  ...
  function Code2Str(code: BGRALongInt): Pstr;
  begin
...
var // GIF buffer can be up to 255 bytes long
  OutputBufferSize: BGRANativeInt;
...
var
  BitBuffer       : BGRADWord; // steady stream of bit output
...
begin
   ...
   //input
   PInput := AImageData;
   PInputEnd := AImageData + BGRAPtrInt(AImageWidth)*AImageHeight;
...
var
  ...
  GIFBlockID:   {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF};
