...
uses
  windows, Classes, SysUtils, Types, {$IFnDEF FPC}Controls, FPImage, {$ENDIF} Graphics, Math;
...
const
  fsSurface = Graphics.fsSurface; //type is defined as Graphics.TFillStyle
  fsBorder = Graphics.fsBorder;
  LineEnding = #13#10;
  LFNSupport = true;
  DirectorySeparator = '\';
  DriveSeparator = ':';
 ...
 { FileNameCaseSensitive and FileNameCasePreserving are defined separately below!!! }
  ...
  MaxPathLen = 260;
  AllFilesMask = '*';
  bvNone    = Controls.bvNone;
  bvLowered = Controls.bvLowered;
  bvRaised  = Controls.bvRaised;
  bvSpace   = Controls.bvSpace;

type
  EFormatError = class(Exception);

  ValReal  = FPImage.ValReal;
  PtrInt   = FPImage.PtrInt;
  PtrUInt  = FPImage.PtrUInt;
  QWord    = FPImage.QWord;
  PPtrInt  = FPImage.PPtrInt;
  PPtrUInt = FPImage.PPtrUInt;
  PQWord   = FPImage.PQWord;
  PDWord   = FPImage.PDWord;
  PUnicodeChar = FPImage.PUnicodeChar;
  UnicodeChar = FPImage.UnicodeChar;

  TGraphicsBevelCut =  Controls.TBevelCut;
...
type
  ...
  TRawImageDescription = {$IFDEF FPC}object{$ELSE}record{$ENDIF}
  ...
  TRawImageLineStarts = {$IFDEF FPC}object{$ELSE}record{$ENDIF}
  private
    FWidth: Cardinal;
    FHeight: Cardinal;
    FBitsPerPixel: Byte;
...
begin
  ...
  then begin
    ...
    FillChar(DstPtr[0], ASize, 0);
  end
  else begin
    LineBytes := Min(RS, ARowStride);
    if H <> LineCount
    then FillChar(DstPtr[0], ASize, 0);
  end;
...
procedure WriteStr(out S: string;  Args: integer);
var
  F : Text;
begin
  Rewrite(F);
...
function TRawImageDescription.AsString: string;
  ...
  function EnumToString(AEnum: TRawImageColorFormat): string; overload;
  begin
    WriteStr(Result, Integer(AEnum));
  end;

  function EnumToString(AEnum: TRawImageByteOrder): string; overload;
  begin
    WriteStr(Result, Integer(AEnum));
  end;

  function EnumToString(AEnum: TRawImageBitOrder): string; overload;
  begin
    WriteStr(Result, Integer(AEnum));
  end;

  function EnumToString(AEnum: TRawImageLineEnd): string; overload;
  begin
    WriteStr(Result, Integer(AEnum));
  end;

  function EnumToString(AEnum: TRawImageLineOrder): string; overload;
  begin
    WriteStr(Result, Integer(AEnum));
  end;
