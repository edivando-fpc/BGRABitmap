...
uses
  Classes, Types, SysUtils, Laz_AVL_Tree;
...
type
  ...
  public
    constructor Create(TheCaseSensitive: boolean); overload;
    constructor Create(const ACompareItems, ACompareNameWithItem: TListSortCompare;
                       TheCaseSensitive: boolean = false); overload;
    destructor Destroy; override;
  ...
  public
    ...
    procedure Add(const Name, Value: string); overload; inline;
    procedure Add(const Name, Value, Delimiter: string); overload;
...
function ComparePointer(Data1, Data2: Pointer): integer;
begin
  if PSizeInt(Data1) > PSizeInt(Data2) then Result:=-1
  else if PSizeInt(Data1) < PSizeInt(Data2) then Result:=1
  else Result:=0;
end;
