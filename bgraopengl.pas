...
function TBGLTexture.CreateOpenGLTexture(ARGBAData: PBGRADWord;
...
procedure TBGLTexture.UpdateOpenGLTexture(ATexture: TBGLTextureHandle;
  ARGBAData: PBGRADWord; AAllocatedWidth, AAllocatedHeight, AActualWidth,
...
var
  m : TAffineMatrix;
begin
  m := AffMatrixMult(AffMatrixMult(AffineMatrixTranslation(rotationCenter.X,rotationCenter.Y),
       AffineMatrixRotationDeg(angleDeg)),
       AffineMatrixTranslation(-rotationCenter.X,-rotationCenter.Y));
  DoDraw(AffMatrixMult(m,PointF(x, y)), AffMatrixMult(m,PointF(x+w, y)), AffMatrixMult(m,PointF(x+w, y+h)), AffMatrixMult(m,PointF(x, y+h)), AColor);
end;
