...
uses
  ...
  AvgLvlTree, BGRABitmapTypes, BGRACanvas2D, BGRATransform;
...
type
  ...
  protected
    FIdentifier: string;
    procedure WriteHeader(AStream: TStream; AName: string; AContentSize: BGRALongInt);
    class procedure ReadHeader(AStream: TStream; out AName: string; out AContentSize: BGRALongInt);
  ...
  public
    Width,Height: single;
    constructor Create(AIdentifier: string); overload; virtual;
  ...
  public
    ...
    procedure NeedGlyphRange(AUnicodeFrom, AUnicodeTo: BGRACardinal);
...
begin
  ...
  nextMove := true;
  AMatrix := AffMatrixMult(AMatrix,AffineMatrixTranslation(Offset.X,Offset.Y));
      ...
      begin
        ...
        begin
          if not isCurvedToPrevious then
            ...
            startCoord := Center;
          ADest.moveTo(AffMatrixMult(AMatrix,startCoord));
          nextMove := false;
        end else
          if not isCurvedToPrevious then
            ADest.lineTo(AffMatrixMult(AMatrix,Points[i]));

        if isCurvedToNext then
        begin
          if not isCurvedToPrevious then ADest.lineTo(AffMatrixMult(AMatrix,Center));
          ADest.quadraticCurveTo(AffMatrixMult(AMatrix,ControlPoint),AffMatrixMult(AMatrix,NextCenter));
        end;
      end;
    end else
    begin
      if nextMove then
      begin
        ADest.moveTo(AffMatrixMult(AMatrix,Points[i]));
        nextMove := false;
      end else
      begin
        ADest.lineTo(AffMatrixMult(AMatrix,Points[i]));
...
var lName: string;
  lContentSize: BGRALongInt;
  EndPosition: BGRAInt64;
begin
  ...
  begin
    ...
    begin
      if AAlign in [twaLeft,twaMiddle,twaRight] then
        m2 := AffMatrixMult(m,AffineMatrixTranslation(0,-g.Height/2)) else
      if AAlign in [twaBottomLeft,twaBottom,twaBottomRight] then
        m2 := AffMatrixMult(m,AffineMatrixTranslation(0,-g.Height))
      else
        m2 := m;
      g.Path(ADest, m2);
      m := AffMatrixMult(m,AffineMatrixTranslation(g.Width,0));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    result := TAffineBox.AffineBox(AffMatrixMult(m,PointF(0,0)),AffMatrixMult(m,PointF(g.Width,0)),AffMatrixMult(m,PointF(0,g.Height)));
  end;
end;
...
begin
  ...
  begin
    ...
    result := TAffineBox.AffineBox(AffMatrixMult(m,PointF(0,minY)),AffMatrixMult(m,PointF(totalWidth,minY)),AffMatrixMult(m,PointF(0,maxY)));
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        result[numChar].Box := TAffineBox.AffineBox(AffMatrixMult(m,PointF(0,gMinY)),AffMatrixMult(m,PointF(g.Width,gMinY)),AffMatrixMult(m,PointF(0,gMaxY)));
        m := AffMatrixMult(m,AffineMatrixTranslation(g.Width,0));
      end else
...
procedure TBGRACustomTypeWriter.NeedGlyphRange(AUnicodeFrom, AUnicodeTo: BGRACardinal);
var c: BGRACardinal;
begin
...
begin
  ...
  tGlyph := PointF(0,0);
  if AAlign in [twaTop,twaMiddle,twaBottom] then tGlyph.X := tGlyph.X -(AGlyph.Width/2);
  if AAlign in [twaTopRight,twaRight,twaBottomRight] then tGlyph.X := tGlyph.X - AGlyph.Width;
  if AAlign in [twaLeft,twaMiddle,twaRight] then tGlyph.Y := tGlyph.Y -(AGlyph.Height/2);
  if AAlign in [twaBottomLeft,twaBottom,twaBottomRight] then tGlyph.Y := tGlyph.Y - AGlyph.Height;
  result := AffMatrixMult(AffMatrixMult(AffineMatrixTranslation(X,Y),TypeWriterMatrix),AffineMatrixTranslation(tGlyph.X,tGlyph.Y));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      g := GetGlyph(nextchar);
      if g <> nil then totalWidth := totalWidth +g.Width;
    end;

    if AAlign in[twaTop,twaMiddle,twaBottom] then tGlyph.X := tGlyph.X -(totalWidth/2) else
    if AAlign in[twaTopRight, twaRight, twaBottomRight] then tGlyph.X := tGlyph.X -totalWidth;
  end;
  result := AffMatrixMult(AffMatrixMult(AffineMatrixTranslation(X,Y),TypeWriterMatrix),AffineMatrixTranslation(tGlyph.X,tGlyph.Y));
end;
