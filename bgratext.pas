...
uses
  Classes, Types, SysUtils, BGRATypes, BGRAGraphics, BGRABitmapTypes, {$IFDEF FPC}InterfaceBase, LCLVersion,{$ENDIF} BGRAPen, BGRAGrayscaleMask;
...
uses GraphType, Math, BGRABlend, BGRAUTF8, BGRAUnicode, BGRATextBidi
     {$IFDEF FPC}{$IFNDEF LCL1070000} , lclplatformdef{$ENDIF} {$ENDIF};
...
begin
  ...
  for i := 0 to FontPixelMetricCount-1 do
    sum := sum +FontPixelMetricArray[i].usage;
...
begin
  ...
  for i := startPos to endPos-1 do
    if (FontPixelMetricArray[i].bold =  {$IFDEF BDS}(fsBold in AFont.Style){$ELSE}AFont.bold  {$ENDIF}) and
      (FontPixelMetricArray[i].italic = {$IFDEF BDS}(fsItalic in AFont.Style){$ELSE}AFont.Italic{$ENDIF}) then
    begin
  ...
  begin
    italic := {$IFDEF BDS}(fsItalic in AFont.Style){$ELSE}AFont.Italic{$ENDIF};
    bold := {$IFDEF BDS}(fsBold in AFont.Style){$ELSE}AFont.bold  {$ENDIF};
    usage := 1;
    name := AFont.Name;
    height:= AFont.Height;
...
begin
  ...
  if not LCLFontDisabledValue {$IFDEF FPC}and not (WidgetSet.LCLPlatform = lpNoGUI){$ENDIF} then
  begin
    lclBmp := TBitmap.Create;
    lclBmp.Canvas.FillRect{$IFDEF BDS}(Rect(0,0,lclBmp.Width,lclBmp.Height)){$ELSE}(0,0,lclBmp.Width,lclBmp.Height){$ENDIF} ;
...
begin
  ...
  if not LCLFontAvailable then
  begin
    {$IFDEF BDS}result.cx := 0; result.cy:= 0;{$ELSE}result := Size(0,0);{$ENDIF}
  end
  else
  begin
    try
      ...
      Result.cx := 0;
      Result.cy := 0;
      tempBmp.Canvas.{$IFDEF FPC}Font.{$ENDIF}GetTextSize(sUTF8, Result.cx, Result.cy);
      if Font.Orientation <> 0 then
      begin
        tempBmp.Canvas.Font.Orientation:= 0;
        extraVerticalMarginDueToRotation := result.cy - tempBmp.Canvas.{$IFDEF FPC}Font.{$ENDIF}GetTextHeight(sUTF8);
      end;
    except
      on ex: exception do
      begin
...
begin
  ...
  if extraMargin > 0 then result.cy := result.cy -extraMargin;
  {$ENDIF}
end;
...
begin
  ...
  case align of
    taLeftJustify: ;
    taCenter: xf := xf -(size.cx/2/sizeFactor);
    taRightJustify: xf := xf -(size.cx/sizeFactor);
  end;
...
begin
  ...
  xf := xf +(TopLeft.x/sizeFactor);
  yf := yf +(TopLeft.y/sizeFactor);
  ...
  xf := xf +(Left/sizeFactor);
  yf := yf +(Top/sizeFactor );
  case align of
    taLeftJustify: ;
    taCenter:
      begin
        xf := xf -(TopRight.x/2/sizeFactor);
        yf := yf -(TopRight.y/2/sizeFactor);
      end;
    taRightJustify:
      begin
        xf := xf -(TopRight.x/sizeFactor);
        yf := yf -(TopRight.y/sizeFactor);
      end;
  end;
  ...
  y := floor(yf);
  if deltaX <> 0 then rotBounds.Right := rotBounds.Right +sizeFactor;
  if deltaY <> 0 then rotBounds.Bottom := rotBounds.Bottom +sizeFactor;
...
begin
  ...
  if Assigned(FWordBreakHandler) then
    ...
    WordBreakHandler := {$IFDEF OBJ}@{$ENDIF}DefaultWorkBreakHandler;
...
begin
  ...
  repeat
    ...
    if not lineEndingBreak then
      ...
      case GetFirstStrongBidiClassUTF8(remains) of
        ubcLeftToRight: if ARightToLeft then part := part +UnicodeCharToUTF8($200E);
        ubcRightToLeft,ubcArabicLetter: if not ARightToLeft then part := part +UnicodeCharToUTF8($200F);
      end;
    lines.Add(part);
  ...
  X := X -round(stepX*lineShift);
  Y := Y -round(stepY*lineShift);
...
begin
  ...
  begin
    ...
    begin
      if style.Layout = tlCenter then Y := Y -InternalTextSize(sUTF8,style.ShowPrefix).cy div 2;
      if style.Layout = tlBottom then Y := Y -InternalTextSize(sUTF8,style.ShowPrefix).cy;
    ...
    begin    //multiple lines
      ...
      if style.Layout = tlCenter then Y := Y -(h*lines.Count) div 2;
      if style.Layout = tlBottom then Y := Y -(h*lines.Count);
      for i := 0 to lines.Count-1 do
      begin
        if style.EndEllipsis then
...
begin
  ...
  begin
    InternalSplitText(sUTF8, round(availableWidth - InternalTextSize('...',AShowPrefix).cx), remain,TWordBreakHandler(nil));
    sUTF8 := sUTF8 +'...';
  end;
