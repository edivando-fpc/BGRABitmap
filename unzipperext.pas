...
uses
  Classes, SysUtils, BGRATypes, zipper;
...
begin
  ...
  Try
    ...
    begin
      ...
      begin
        OnCreateStream := {$IFDEF OBJ}@{$ENDIF}CustomCreateOutput;
        OnDoneStream := {$IFDEF OBJ}@{$ENDIF}CustomCloseOutput;
...
begin
  inherited Create;
  OnOpenInputStream := {$IFDEF OBJ}@{$ENDIF}CustomOpenInput;
  OnCloseInputStream:= {$IFDEF OBJ}@{$ENDIF}CustomCloseInput;
end;
