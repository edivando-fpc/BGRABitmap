...
uses
  Classes, SysUtils, FPImage, BGRATypes,{$IFNDEF FPC}Types, GraphType, BGRAGraphics,{$ENDIF} FPReadPSD;
...
type
  ...
  protected
    ...
    function ReadScanLine(Stream: TStream; AInputSize: BGRAPtrInt; AChannel: integer): boolean; overload;
...
begin
  ...
  begin
    ...
    FLineSize:=(BGRAPtrInt(FWidth)*Depth+7) div 8;
...
var
  ...
  LenOfLineFactor: BGRAPtrInt;
  BufSize:BGRACardinal;
  Encoding:BGRAWord;
  ContProgress: Boolean;
  CurOffset: BGRAInt64;
...
function TBGRAReaderPSD.ReadScanLine(Stream: TStream; AInputSize: BGRAPtrInt;
...
function Value32To16(p: PBGRADWord; gamma: single): BGRAWord;
var v: single;
begin
    ...
    case FBytesPerPixel of
      ...
      16 : if (FHeader.Mode = 1) or (FHeader.Mode = 8) then
           ...
           begin
             for Col:=0 to Img.Width-1 do
             begin
               Img.Colors[Col,Row]:=ThePalette[BEtoN(PBGRAWord(P)^)];
               inc(p,2);
             end;
          end;
      ...
      32 : if (FHeader.Mode = 1) or (FHeader.Mode = 8) then
           begin
             if FChannelCount = 1 then
               for Col:=0 to Img.Width-1 do
               begin
                 C.Red:=Value32To16(PBGRADWord(P),1.3);
                 C.green:=C.Red;
                 C.blue:=C.Red;
                 C.alpha:=65535;
                 Img[col, row] := C;
      ...
      48 :if FChannelCount = 3 then
          begin
           ...
           begin
              if (FHeader.Mode =9) then
                C := LabToRGB(BEtoN(PBGRAWord(P)^)/65535, (BEtoN(PBGRAWord(P1)^)-32768)/32767, (BEtoN(PBGRAWord(P2)^)-32768)/32767)
              else
              With C do
              begin
                Red:=BEtoN(PBGRAWord(P)^);
                green:=BEtoN(PBGRAWord(P1)^);
                blue:=BEtoN(PBGRAWord(P2)^);
              end;
              Inc(P,2);
              Inc(P1,2);
              Inc(P2,2);
      ...
      64 : if FChannelCount = 4 then
           ...
           begin
             C.Red:=BEtoN(PBGRAWord(P)^);
             C.green:=BEtoN(PBGRAWord(P1)^);
             C.blue:=BEtoN(PBGRAWord(P2)^);
             C.alpha:=BEtoN(PBGRAWord(P3)^);
             if (FHeader.Mode =4) then
             begin
                 C.red:=$ffff-C.red;
      ...
      96 :if FChannelCount = 3 then
          begin
           ...
           begin
              With C do
              begin
                Red:=Value32To16(PBGRADWord(P),2.7);
                green:=Value32To16(PBGRADWord(P1),2.7);
                blue:=Value32To16(PBGRADWord(P2),2.7);
              end;
              Inc(P,4);
              Inc(P1,4);
              Inc(P2,4);
