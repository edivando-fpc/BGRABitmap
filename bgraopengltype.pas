...
uses
  Classes, SysUtils, Types, BGRATypes,{$IFNDEF FPC}GraphType, {$ENDIF} BGRABitmap, BGRABitmapTypes,
  FPImage, BGRATransform,
  BGRASSE, BGRAMatrix3D, BGRAGraphics;
...
type
  ...
  IBGLFont = interface
    ['{E415DCF3-C4D7-4625-AF0B-2E756432CDE7}']
  ...
  IBGLTexture = interface
    ['{BF2FF051-EBC6-4102-8268-37A9D0297B92}']
    ...
    procedure SetGradientColors(ATopLeft, ATopRight, ABottomRight, ABottomLeft: TBGRAPixel);
    procedure SetUseGradientColors(AValue: boolean);
    procedure Update(ARGBAData: PBGRADWord; AllocatedWidth, AllocatedHeight, ActualWidth,ActualHeight: integer; RGBAOrder: boolean = true);
  ...
  protected
    ...
    function GetOpenGLMaxTexSize: integer; virtual; abstract;
    function CreateOpenGLTexture(ARGBAData: PBGRADWord; AAllocatedWidth, AAllocatedHeight, AActualWidth, AActualHeight: integer; RGBAOrder: boolean): TBGLTextureHandle; virtual; abstract;
    procedure UpdateOpenGLTexture(ATexture: TBGLTextureHandle; ARGBAData: PBGRADWord; AAllocatedWidth, AAllocatedHeight, AActualWidth,AActualHeight: integer; RGBAOrder: boolean); virtual; abstract;
    ...
    procedure InitEmpty;
    procedure InitFromData(ARGBAData: PBGRADWord; AllocatedWidth,AllocatedHeight, ActualWidth,ActualHeight: integer; RGBAOrder: boolean);
  ...
  public
    ...
    constructor Create(ARGBAData: PBGRADWord; AllocatedWidth,AllocatedHeight, ActualWidth,ActualHeight: integer; RGBAOrder: boolean = true); overload;
...
procedure TBGLCustomTexture.Update(ARGBAData: PBGRADWord; AllocatedWidth,
...
procedure TBGLCustomTexture.InitFromData(ARGBAData: PBGRADWord;
...
begin
  ...
  try
    ...
    InitFromData(PBGRADWord(bmp.Data), bmp.AllocatedWidth,bmp.AllocatedHeight, bmp.Width,bmp.Height,TBGRAPixel_RGBAOrder or not SupportsBGRAOrder);
  except
    InitEmpty;
  end;
  bmp.Free;
...
constructor TBGLCustomTexture.Create(ARGBAData: PBGRADWord; AllocatedWidth,
  AllocatedHeight, ActualWidth, ActualHeight: integer; RGBAOrder: boolean);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      InitFromData(PBGRADWord(Data), Width,Height, Width,Height, TBGRAPixel_RGBAOrder or not SupportsBGRAOrder);
      if LineOrder = riloBottomToTop then VerticalFlip;
      if not TBGRAPixel_RGBAOrder and not SupportsBGRAOrder then SwapRedBlue;
    end;
  end else
...
begin
  ...
  try
    ...
    if not TBGRAPixel_RGBAOrder and not SupportsBGRAOrder then bmp.SwapRedBlue;
    InitFromData(PBGRADWord(bmp.Data), bmp.AllocatedWidth,bmp.AllocatedHeight, bmp.Width,bmp.Height, TBGRAPixel_RGBAOrder);
  except
    InitEmpty;
    NotifyErrorLoadingFile(AFilenameUTF8);
  end;
...
begin
  DoDrawAffine(AffMatrixMult(AMatrix,PointF(0,0)) + PointF(x,y), AffMatrixMult(AMatrix,PointF(Width,0)) + PointF(x,y),
     AffMatrixMult(AMatrix,PointF(0,Height)) + PointF(x,y), BGRA(255,255,255,AAlpha));
end;
...
begin
  DoDrawAffine(AffMatrixMult(AMatrix,PointF(0,0)) + PointF(x,y), AffMatrixMult(AMatrix,PointF(Width,0)) + PointF(x,y),
     AffMatrixMult(AMatrix,PointF(0,Height)) + PointF(x,y), AColor);
end;
  ...
  begin
    ...
    begin
      ...
      if FTexture = nil then
        FTexture := BGLTextureFactory.Create(PBGRADWord(self.Data), AllocatedWidth,AllocatedHeight, Width,Height, TBGRAPixel_RGBAOrder or not BGLTextureFactory.SupportsBGRAOrder)
      else
        FTexture.Update(PBGRADWord(self.Data), AllocatedWidth,AllocatedHeight, Width,Height, TBGRAPixel_RGBAOrder or not BGLTextureFactory.SupportsBGRAOrder);
      if not TBGRAPixel_RGBAOrder and not BGLTextureFactory.SupportsBGRAOrder then SwapRedBlueWithoutInvalidate(Rect(0,0,Width,Height));
    end;
    result := FTexture;
  end;
