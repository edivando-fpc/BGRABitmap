...
begin
  ...
  begin
    ...
    begin
      currentState.fontStyle := currentState.fontStyle +[fsItalic];
    end else
    if (attrib = 'bold') or (attrib = 'bolder') then
    begin
      currentState.fontStyle := currentState.fontStyle +[fsBold];
    end else
    ...
    begin
      ...
      begin
        if u = '' then //weight
        begin
          if value >= 600 then currentState.fontStyle := currentState.fontStyle +[fsBold];
        end else
...
var
  ...
  contour : {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
...
{$IFDEF FPC}
function TBGRACanvas2D.QueryInterface({$IFDEF FPC_HAS_CONSTREF}constref{$ELSE}const{$ENDIF} IID: TGUID; out Obj): HResult; {$IF (not defined(WINDOWS)) AND (not defined(FPC20501))}cdecl{$ELSE}stdcall{$IFEND};
{$ELSE}
function TBGRACanvas2D.QueryInterface(const IID: TGUID; out Obj): HResult; stdcall;
{$ENDIF}
begin
...
{$IFDEF FPC}
function TBGRACanvas2D._AddRef: Integer; {$IF (not defined(WINDOWS)) AND (not defined(FPC20501))}cdecl{$ELSE}stdcall{$IFEND};
{$ELSE}
function TBGRACanvas2D._AddRef: Integer; stdcall;
{$ENDIF}
begin
  result := 0;
end;
...
{$IFDEF FPC}
function TBGRACanvas2D._Release: Integer; {$IF (not defined(WINDOWS)) AND (not defined(FPC20501))}cdecl{$ELSE}stdcall{$IFEND};
{$ELSE}
function TBGRACanvas2D._Release: Integer; stdcall;
{$ENDIF}
begin
  result := 0;
end;
...
procedure TBGRACanvas2D.DrawShadow(const points, points2: array of TPointF; AFillMode: TFillMode = fmWinding);
{$IFDEF OBJ}
...
{$ENDIF}
  ...
  begin
    ...
    for i := 0 to high(ofsPts) do
      ofsPts[i] := ofsPts[i] +offset;
    for i := 0 to high(ofsPts2) do
      ofsPts2[i] := ofsPts2[i] +offset;
  end;
...
begin
  ...
  for i := 0 to high(result) do
    if isEmptyPointF(points[i]) then
      ...
      result[i] := AffMatrixMult(matrix,points[i])+FCanvasOffset;
end;
...
begin
  ...
  for i := 0 to high(result) do
    if isEmptyPointF(points[i]) then
      ...
      result[i] := AffMatrixMult(currentState.matrix,points[i])+FCanvasOffset;
end;

function TBGRACanvas2D.ApplyTransform(point: TPointF): TPointF;
begin
  result := AffMatrixMult(currentState.matrix,point)+FCanvasOffset;
end;
...
begin
  currentState.matrix := AffMatrixMult(currentState.matrix, AffineMatrixScale(x,y));
end;

procedure TBGRACanvas2D.scale(factor: single);
begin
  currentState.matrix := AffMatrixMult(currentState.matrix, AffineMatrixScale(factor,factor));
end;

procedure TBGRACanvas2D.rotate(angleRadCW: single);
begin
  currentState.matrix := AffMatrixMult(currentState.matrix, AffineMatrixRotationRad(-angleRadCW));
end;

procedure TBGRACanvas2D.translate(x, y: single);
begin
  if (x = 0) and (y = 0) then exit;
  currentState.matrix := AffMatrixMult(currentState.matrix, AffineMatrixTranslation(x,y));
end;

procedure TBGRACanvas2D.skewx(angleRadCW: single);
begin
  currentState.matrix := AffMatrixMult(currentState.matrix, AffineMatrixSkewXRad(-angleRadCW));
end;

procedure TBGRACanvas2D.skewy(angleRadCW: single);
begin
  currentState.matrix := AffMatrixMult(currentState.matrix, AffineMatrixSkewYRad(-angleRadCW));
end;

procedure TBGRACanvas2D.transform(m11,m21, m12,m22, m13,m23: single);
begin
  currentState.matrix := AffMatrixMult(currentState.matrix, AffineMatrix(m11,m12,m13,
                                      m21,m22,m23));
end;

procedure TBGRACanvas2D.transform(AMatrix: TAffineMatrix);
begin
  currentState.matrix := AffMatrixMult(currentState.matrix, AMatrix);
end;
...
begin
  currentState.penStroker.strokeMatrix := AffMatrixMult(currentState.penStroker.strokeMatrix, AffineMatrixScale(x,y));
end;

procedure TBGRACanvas2D.strokeSkewx(angleRadCW: single);
begin
  currentState.penStroker.strokeMatrix := AffMatrixMult(currentState.penStroker.strokeMatrix, AffineMatrixSkewXRad(-angleRadCW));
end;

procedure TBGRACanvas2D.strokeSkewy(angleRadCW: single);
begin
  currentState.penStroker.strokeMatrix := AffMatrixMult(currentState.penStroker.strokeMatrix, AffineMatrixSkewYRad(-angleRadCW));
end;
...
begin
  result := TBGRACanvasLinearGradient2D.Create(p0,p1,
            AffMatrixMult(AffineMatrixTranslation(FCanvasOffset.x,FCanvasOffset.y),currentState.matrix));
...
begin
  tempTransform := AffMatrixMult(AffineMatrixTranslation(FCanvasOffset.X+0.5,FCanvasOffset.Y+0.5),currentState.matrix);
...
var i,j: integer;
  pts, splinePts: {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
  nb: integer;
begin
  if FPathPointCount > 0 then
  begin
...
begin
  ...
  pt := AffMatrixMult(currentState.matrix, PointF(x,y));
