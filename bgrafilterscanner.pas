...
uses
  Classes, BGRATypes, {$IFNDEF FPC}Types, GraphType,{$ENDIF} BGRAGraphics, BGRABitmapTypes, BGRAFilterType;
...
type
  ...
  private
    ...
    minValBlue, maxValBlue, minAlpha, maxAlpha: BGRAWord;
    addValRed, addValGreen, addValBlue, addAlpha: BGRAWord;
...
begin
  sum := BGRANativeInt((PByte(PTop)+FChannelOffset)^) +
         BGRANativeInt((PByte(PTop+1)+FChannelOffset)^) +
         BGRANativeInt((PByte(PMiddle)+FChannelOffset)^) -
         BGRANativeInt((PByte(PMiddle+2)+FChannelOffset)^) -
         BGRANativeInt((PByte(PBottom+1)+FChannelOffset)^) -
         BGRANativeInt((PByte(PBottom+2)+FChannelOffset)^);
  sum := 128 - sum div 3;
  if sum > 255 then
    slope := 255
  else
...
var
  sumR, sumG, sumB, sumA, nbA: BGRANativeUInt;
  refPixel: TBGRAPixel;
  rgbDivShr1: BGRANativeUInt;
begin
  if FAmount = 0 then
  begin
    result := PMiddle[1];
  ...
  {$hints off}
  with PTop[0] do if alpha <> 0 then begin sumR := sumR +(red * alpha); sumG := sumG +(green * alpha); sumB := sumB +(blue * alpha); sumA := sumA +alpha; inc(nbA); end;
  with PTop[1] do if alpha <> 0 then begin sumR := sumR +(red * alpha); sumG := sumG +(green * alpha); sumB := sumB +(blue * alpha); sumA := sumA +alpha; inc(nbA); end;
  with PTop[2] do if alpha <> 0 then begin sumR := sumR +(red * alpha); sumG := sumG +(green * alpha); sumB := sumB +(blue * alpha); sumA := sumA +alpha; inc(nbA); end;
  with PMiddle[0] do if alpha <> 0 then begin sumR := sumR +(red * alpha); sumG := sumG +(green * alpha); sumB := sumB +(blue * alpha); sumA := sumA +alpha; inc(nbA); end;
  with PMiddle[2] do if alpha <> 0 then begin sumR := sumR +(red * alpha); sumG := sumG +(green * alpha); sumB := sumB +(blue * alpha); sumA := sumA +alpha; inc(nbA); end;
  with PBottom[0] do if alpha <> 0 then begin sumR := sumR +(red * alpha); sumG := sumG +(green * alpha); sumB := sumB +(blue * alpha); sumA := sumA +alpha; inc(nbA); end;
  with PBottom[1] do if alpha <> 0 then begin sumR := sumR +(red * alpha); sumG := sumG +(green * alpha); sumB := sumB +(blue * alpha); sumA := sumA +alpha; inc(nbA); end;
  with PBottom[2] do if alpha <> 0 then begin sumR := sumR +(red * alpha); sumG := sumG +(green * alpha); sumB := sumB +(blue * alpha); sumA := sumA +alpha; inc(nbA); end;
   {$hints on}
  ...
  if not BGRAPixelEqual(refPixel, {<>} BGRAPixelTransparent) then
  begin
    with PMiddle[1] do
    begin
