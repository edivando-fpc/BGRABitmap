...
uses
  Math, Classes, SysUtils, ctypes, zinflate, zbase, {$IFNDEF FPC}Types, GraphType, BGRAGraphics,{$ENDIF}BGRATypes, FPImage, FPTiffCmn,
  BGRABitmapTypes;
...
type
  ...
  private
    ...
    procedure SetStreamPos(p: BGRADWord);
    function ReadTiffHeader(QuickTest: boolean; out IFDStart: BGRADWord): boolean; // returns IFD: offset to first IFD
    function ReadIFD(Start: BGRADWord; IFD: TTiffIFD): BGRADWord;// Image File Directory
    procedure ReadDirectoryEntry(var EntryTag: BGRAWord; IFD: TTiffIFD);
    function ReadEntryUnsigned: BGRADWord;
    function ReadEntrySigned: Cint32;
    function ReadEntryRational: TTiffRational;
    function ReadEntryString: string;
    function ReadByte: Byte;
    function ReadWord: BGRAWord;
    function ReadDWord: BGRADWord;
    procedure ReadValues(StreamPos: BGRADWord;
                         out EntryType: BGRAWord; out EntryCount: BGRADWord;
                         out Buffer: Pointer; out ByteCount: BGRAPtrUInt);
    procedure ReadShortOrLongValues(StreamPos: BGRADWord;
                                    out Buffer: PBGRADWord; out Count: BGRADWord);
    procedure ReadShortValues(StreamPos: BGRADWord;
                              out Buffer: PBGRAWord; out Count: BGRADWord);
    procedure ReadImageSampleProperties(IFD: TTiffIFD; out AlphaChannel: integer; out PremultipliedAlpha: boolean;
      out SampleCnt: BGRADWord; out SampleBits: PBGRAWord; out SampleBitsPerPixel: BGRADWord;
      out PaletteCnt: BGRADWord; out PaletteValues: PBGRAWord);
    procedure ReadImgValue(BitCount: BGRAWord;
      var Run: Pointer; var BitPos: Byte; FillOrder: BGRADWord;
      Predictor: BGRAWord; var LastValue: BGRAWord; out Value: BGRAWord);
    function FixEndian(w: BGRAWord): BGRAWord;{$ifdef inline}inline;{$endif}overload;
    function FixEndian(d: BGRADWord): BGRADWord;{$ifdef inline}inline;{$endif}overload;
    procedure SetFPImgExtras(CurImg: TFPCustomImage; IFD: TTiffIFD);
    procedure DecodePackBits(var Buffer: Pointer; var Count: BGRAPtrInt);
    procedure DecodeLZW(var Buffer: Pointer; var Count: BGRAPtrInt);
    procedure DecodeDeflate(var Buffer: Pointer; var Count: BGRAPtrInt; ExpectedCount: BGRAPtrInt);
  protected
  ...
  public //advanced
    ...
    procedure LoadImageFromStream(Index: integer); overload; // call LoadIFDsFromStream before
    procedure LoadImageFromStream(IFD: TTiffIFD); overload; // call LoadIFDsFromStream before
    procedure ReleaseStream;
    property StartPos: BGRAInt64 read fStartPos;
    property ReverserEndian: boolean read FReverserEndian;
    property TheStream: TStream read s;
    property FirstIFDStart: BGRADWord read FFirstIFDStart;
  end;

procedure DecompressPackBits(Buffer: Pointer; Count: BGRAPtrInt;
  out NewBuffer: Pointer; out NewCount: BGRAPtrInt);
procedure DecompressLZW(Buffer: Pointer; Count: BGRAPtrInt;
  out NewBuffer: PByte; out NewCount: BGRAPtrInt);
function DecompressDeflate(Compressed: PByte; CompressedCount: BGRACardinal;
  out Decompressed: PByte; var DecompressedCount: BGRACardinal;
  ErrorMsg: PAnsiString = nil): boolean;

implementation

function CMYKToFPColor(C,M,Y,K: BGRAWord): TFPColor;
var R, G, B : BGRALongWord;
begin
   R := $ffff - ((BGRALongWord(C)*($ffff-BGRALongWord(K))) shr 16) - BGRALongWord(K) ;
   G := $ffff - ((BGRALongWord(M)*($ffff-BGRALongWord(K))) shr 16) - BGRALongWord(K) ;
   B := $ffff - ((BGRALongWord(Y)*($ffff-BGRALongWord(K))) shr 16) - BGRALongWord(K) ;
...
procedure TBGRAReaderTiff.ReadImageSampleProperties(IFD: TTiffIFD;
  ...
  out PaletteCnt: BGRADWord; out PaletteValues: PBGRAWord);
var
  BytesPerPixel: BGRAWord;
  i: Integer;
  ExtraSampleCnt, RegularSampleCnt: BGRADWord;
  ExtraSamples: PBGRAWord;
begin
...
procedure TBGRAReaderTiff.ReadImgValue(BitCount: BGRAWord;
  var Run: Pointer; var BitPos: Byte; FillOrder: BGRADWord;
  Predictor: BGRAWord; var LastValue: BGRAWord; out Value: BGRAWord);
var
  BitNumber: byte;
...
procedure TBGRAReaderTiff.SetStreamPos(p: BGRADWord);
var
  NewPosition: BGRAInt64;
begin
  NewPosition:=BGRAInt64(p)+fStartPos;
...
begin
  ...
  for i:=0 to ImageCount-1 do begin
    IFD:=Images[i];
    CurSize:=BGRAInt64(IFD.ImageWidth)*IFD.ImageHeight;
...
function TBGRAReaderTiff.ReadTiffHeader(QuickTest: boolean; out IFDStart: BGRADWord): boolean;
var
  ByteOrder: String;
  BigEndian: Boolean;
  FortyTwo: BGRAWord;
...
function TBGRAReaderTiff.ReadIFD(Start: BGRADWord; IFD: TTiffIFD): BGRADWord;
var
  Count: BGRAWord;
  i: Integer;
  EntryTag: BGRAWord;
...
procedure TBGRAReaderTiff.ReadDirectoryEntry(var EntryTag: BGRAWord; IFD: TTiffIFD);
var
  EntryType: BGRAWord;
  EntryCount: BGRADWord;
  EntryStart: BGRADWord;
  NewEntryTag: BGRAWord;
  UValue: BGRADWord;
  SValue: integer;
  WordBuffer: PBGRAWord;
  Count: BGRADWord;
  i: Integer;

  function GetPos: BGRADWord;
  begin
     Result:=BGRADWord(s.Position-fStartPos-2)
  end;
...
begin
  ...
  317:
    begin
      // Predictor
      UValue:=BGRAWord(ReadEntryUnsigned);
      case UValue of
      1: ;
      2: ;
...
function TBGRAReaderTiff.ReadEntryUnsigned: BGRADWord;
var
  EntryCount: BGRALongWord;
  EntryType: BGRAWord;
begin
  Result:=0;
  EntryType:=ReadWord;
...
begin
  ...
  if EntryCount>4 then begin
    // long string -> next 4 BGRADWord is the offset
    EntryStart:=ReadDWord;
    SetStreamPos(EntryStart);
...
function TBGRAReaderTiff.ReadWord: BGRAWord;
begin
  Result:=FixEndian(s.ReadWord);
end;

function TBGRAReaderTiff.ReadDWord: BGRADWord;
begin
  Result:=FixEndian(s.ReadDWord);
end;

procedure TBGRAReaderTiff.ReadValues(StreamPos: BGRADWord; out EntryType: BGRAWord; out
  EntryCount: BGRADWord; out Buffer: Pointer; out ByteCount: BGRAPtrUInt);
var
  EntryStart: BGRADWord;
begin
  Buffer:=nil;
...
procedure TBGRAReaderTiff.ReadShortOrLongValues(StreamPos: BGRADWord; out
  Buffer: PBGRADWord; out Count: BGRADWord);
var
  p: Pointer;
  ByteCount: BGRAPtrUInt;
  EntryType: BGRAWord;
  i: BGRADWord;
begin
  Buffer:=nil;
  ...
  try
    ...
    if EntryType=3 then begin
      // short
      GetMem(Buffer,SizeOf(BGRADWord)*Count);
      for i:=0 to Count-1 do
        Buffer[i]:=FixEndian(PBGRAWord(p)[i]);
      ...
      if FReverseEndian then
        for i:=0 to Count-1 do
          Buffer[i]:=FixEndian(PBGRADWord(Buffer)[i]);
    end else
...
procedure TBGRAReaderTiff.ReadShortValues(StreamPos: BGRADWord; out Buffer: PBGRAWord;
  out Count: BGRADWord);
var
  p: Pointer;
  ByteCount: BGRAPtrUInt;
...
var
  SampleCnt: BGRADWord;
  SampleBits: PBGRAWord;
  ChannelValues, LastChannelValues: array of BGRAWord;

  PaletteCnt,PaletteStride: BGRADWord;
  PaletteValues: PBGRAWord;

  AlphaChannel: integer;
  PremultipliedAlpha: boolean;

  procedure InitColor;
  var Channel: BGRADWord;
  begin
  ...
  var Channel, PaletteIndex: BGRADWord;
    GrayValue: BGRAWord;
  begin
    for Channel := 0 to SampleCnt-1 do
...
var
  ChunkOffsets: PBGRADWord;
  ChunkByteCounts: PBGRADWord;
  Chunk: PByte;
  ChunkCount: BGRADWord;
  ChunkIndex: BGRADWord;
  CurCount: BGRADWord;
  CurOffset: BGRADWord;
  CurByteCnt: BGRAPtrInt;
  Run: PByte;
  BitPos: Byte;
  x, y, cx, cy, dx1,dy1, dx2,dy2, sx: integer;
  SampleBitsPerPixel: BGRADWord;
  CurFPImg: TFPCustomImage;
  aContinue: Boolean;
  ExpectedChunkLength: BGRAPtrInt;
  ChunkType: TTiffChunkType;
  TilesAcross, TilesDown: BGRADWord;
  ChunkLeft, ChunkTop, ChunkWidth, ChunkHeight: BGRADWord;
  ChunkBytesPerLine: BGRADWord;
begin
...
function TBGRAReaderTiff.FixEndian(w: BGRAWord): BGRAWord; {$ifdef inline}inline;{$endif}
begin
  Result:=w;
  if FReverseEndian then
...
function TBGRAReaderTiff.FixEndian(d: BGRADWord): BGRADWord; {$ifdef inline}inline;{$endif}
begin
  Result:=d;
  if FReverseEndian then
...
procedure TBGRAReaderTiff.DecodePackBits(var Buffer: Pointer; var Count: BGRAPtrInt);
var
  NewBuffer: Pointer;
  NewCount: BGRAPtrInt;
begin
...
procedure TBGRAReaderTiff.DecodeLZW(var Buffer: Pointer; var Count: BGRAPtrInt);
var
  NewBuffer: Pointer;
  NewCount: BGRAPtrInt;
begin
...
procedure TBGRAReaderTiff.DecodeDeflate(var Buffer: Pointer; var Count: BGRAPtrInt;
  ExpectedCount: BGRAPtrInt);
var
  NewBuffer: PByte;
  NewCount: BGRACardinal;
  ErrorMsg: String;
begin
  ErrorMsg:='';
  NewBuffer:=nil;
...
procedure DecompressPackBits(Buffer: Pointer; Count: BGRAPtrInt; out
  NewBuffer: Pointer; out NewCount: BGRAPtrInt);
{ Algorithm:
...
procedure DecompressLZW(Buffer: Pointer; Count: BGRAPtrInt; out NewBuffer: PByte;
  out NewCount: BGRAPtrInt);
type
...
const
  ...
  NoCode = $7fff;
var
  NewCapacity: BGRAPtrInt;
  SrcPos: BGRAPtrInt;
  CodeBuffer: BGRADWord;
  ...
  begin
    ...
    begin
      ...
      If BigEndian then
        ...
        CodeBuffer := CodeBuffer or (BGRADWord(PByte(Buffer)[SrcPos]) shl CodeBufferLength);
      Inc(SrcPos);
  ...
  function IsInTable(Code: BGRAWord): boolean;
  begin
