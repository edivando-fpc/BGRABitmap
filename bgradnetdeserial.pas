          ...
          begin
            try
              ...
              begin
                ...
                if subNum = -1 then
                  Result := result +'(Not found) #' + IntToStr(subId)+LineEnding
                else
                  Result := result + objectToString(subNum, fieldTypeStr, tab + '  ', False);
              end;
            except
              result := result +'!' + fieldAsString[j]+'!' +LineEnding
            end;
...
begin
  ...
  for i := 0 to high(assemblies) do
    Result := result +'Imports ' + assemblies[i].Name + LineEnding;
  Result   := result +lineEnding;
  for i := 0 to high(objects) do
    objects[i].inToString := False;
  for i := 0 to high(objects) do
    Result := result +ObjectToString(i, 'Object', '', True);
end;
...
function TDotNetDeserialization.GetTypeOfClassObject(idObject: BGRALongWord
  ): integer;
var
  i: Integer;
begin
...
function TDotNetDeserialization.nextAutoObjectId: BGRALongWord;
begin
  Inc(currentAutoObjectValue);
  Result := currentAutoObjectValue;
end;

function TDotNetDeserialization.LoadNextFromStream(Stream: TStream): BGRALongWord;
var
  blockType:    byte;
  idRefObject, tempIdObject: BGRALongWord;
  tempType:     TFieldType;
  arrayCount, arrayIndex,FillZeroCount : BGRALongWord;
  ...
  genericArrayRank: BGRALongWord;
  genericArrayDims: {$IFDEF BDS}arrayOfLongword{$ELSE}array of BGRALongWord{$ENDIF};
  genericArrayItemType: TFieldType;

  function GetArrayCellNumber(index: BGRALongWord): string;
  var r: BGRALongWord;
  begin
    result := '';
    ...
    begin
      if r <> 1 then result:=result +',';
      result := result +inttostr(index mod genericArrayDims[r-1]);
              ...
              begin
                ...
                begin
                  ...
                  if FillZeroCount > 0 then
                    ...
                    PBGRALongWord(newArrayObj.ItemPtr[arrayIndex])^ := tempIdObject;
                end;
              end;
            end;
        except
