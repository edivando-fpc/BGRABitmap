...
begin
  ...
  if Odd(NbPixels) and (PBGRADWord(p)^ <> 0) then
  begin
    Result := false;
    exit;
  end;
...
begin
  ...
  Case AValue of
  dmLinearBlend: FCanvasPixelProcFP := {$IFDEF OBJ}@{$ENDIF}FastBlendPixel;
  dmDrawWithTransparency: FCanvasPixelProcFP := {$IFDEF OBJ}@{$ENDIF}DrawPixel;
  dmXor: FCanvasPixelProcFP:= {$IFDEF OBJ}@{$ENDIF}XorPixel;
  else FCanvasPixelProcFP := {$IFDEF OBJ}@{$ENDIF}SetPixel;
  end;
end;
...
begin
  ...
  if ACustomOrientation <> 0 then
    result := AffMatrixMult(AffineMatrixRotationDeg(-ACustomOrientation*0.1),result);
end;

{ Get scanline without checking bounds nor updated from TBitmap }
function TBGRADefaultBitmap.GetScanlineFast(y: integer): PBGRAPixel; //@{$ifdef inline}inline;{$endif}
begin
  Result := FData;
...
function TBGRADefaultBitmap.GetPixel256(x, y, fracX256, fracY256: int32or64;
  AResampleFilter: TResampleFilter; smoothBorder: boolean): TBGRAPixel;
begin
...
procedure TBGRADefaultBitmap.DiscardBitmapChange; //@{$ifdef inline}inline;{$endif}
begin
...
function TBGRADefaultBitmap.CheckHorizLineBounds(var x,y,x2: int32or64): boolean; //@{$ifdef inline}inline;{$endif}
var
  temp: int32or64;
begin
  if (x2 < x) then
...
function TBGRADefaultBitmap.CheckVertLineBounds(var x,y,y2: int32or64; out delta: int32or64): boolean; //@{$ifdef inline}inline;{$endif}
var
  temp: int32or64;
begin
  if FLineOrder = riloBottomToTop then
...
begin
  Orig := AffMatrixMult(AMatrix,PointF(ASourceBounds.Left,ASourceBounds.Top));
  ...
  HAxis := AffMatrixMult(AMatrix,PointF(ASourceBounds.Right-1,ASourceBounds.Top));
  ...
  VAxis := AffMatrixMult(AMatrix,PointF(ASourceBounds.Left,ASourceBounds.Bottom-1));
...
procedure TBGRADefaultBitmap.FillTriangleLinearMappingLightness(pt1, pt2,
  ...
  light2, light3: BGRAWord; TextureInterpolation: Boolean);
begin
...
procedure TBGRADefaultBitmap.FillQuadLinearMappingLightness(pt1, pt2, pt3,
  ...
  light2, light3, light4: BGRAWord; TextureInterpolation: Boolean);
var
  center: TPointF;
  centerTex: TPointF;
  centerLight: BGRAWord;
begin
  center := (pt1+pt2+pt3+pt4)*(1/4);
...
begin
  ...
  begin
    Orig  := Orig  -PointF(0.5,0.5);
    HAxis := HAxis -PointF(0.5,0.5);
    VAxis := VAxis -PointF(0.5,0.5);
  end;
...
procedure TBGRADefaultBitmap.FillPolyLinearMappingLightness(
  ...
  texCoords: array of TPointF; lightnesses: array of BGRAWord;
  TextureInterpolation: Boolean);
begin
...
procedure TBGRADefaultBitmap.FillPolyPerspectiveMappingLightness(
  const points: array of TPointF; const pointsZ: array of single;
  texture: IBGRAScanner; texCoords: array of TPointF;
  lightnesses: array of BGRAWord; TextureInterpolation: Boolean; zbuffer: psingle);
begin
...
begin
  ...
  begin
    if not BGRAPixelEqual(back, {<>} BGRAPixelTransparent) then
...
function TBGRADefaultBitmap.CheckRectBounds(var x, y, x2, y2: integer; minsize: integer): boolean; //@{$ifdef inline}inline;{$endif}
var
  temp: integer;
begin
...
function TBGRADefaultBitmap.CheckClippedRectBounds(var x, y, x2, y2: integer): boolean; //@{$ifdef inline}inline;{$endif}
var
  temp: integer;
begin
  if (x > x2) then
...
begin
  ...
  result := TAffineBox.AffineBox(PointF(0,-dy), AffMatrixMult(m,PointF(size.cx,-dy)), AffMatrixMult(m,PointF(0,size.cy-dy)));
end;
...
function TBGRADefaultBitmap.ComputeArc65536(x, y, rx, ry: single; start65536,
  end65536: BGRAWord; quality: single): ArrayOfTPointF;
begin
...
var
  p:  {$IFDEF BDS}PBGRAPixel;{$ELSE}PBGRALongWord;{$ENDIF}//#
  n: integer;
  colorMask,beforeBGR, afterBGR: {$IFDEF BDS}TBGRAPixel;{$ELSE}BGRALongWord;{$ENDIF} //#
  rAfter,gAfter,bAfter,rBefore,gBefore,bBefore: byte;
begin
  colorMask := {$IFDEF BDS}BGRA(255,255,255,0);{$ELSE}BGRALongWord(BGRA(255,255,255,0));{$ENDIF}
  RedGreenBlue(before, rBefore,gBefore,bBefore);
  RedGreenBlue(after, rAfter,gAfter,bAfter);
  beforeBGR := {$IFDEF BDS}BGRA(rBefore,gBefore,bBefore,0);{$ELSE}BGRALongWord(BGRA(rBefore,gBefore,bBefore,0));{$ENDIF}
  afterBGR  := {$IFDEF BDS}BGRA(rAfter,gAfter,bAfter,0);{$ELSE}BGRALongWord(BGRA(rAfter,gAfter,bAfter,0));{$ENDIF}

  p := {$IFDEF BDS}Data;{$ELSE}PBGRALongWord(Data);{$ENDIF}
  for n := NbPixels - 1 downto 0 do
  begin
    {$IFDEF BDS}
    if {#}BGRALongWord(p^) and colorMask.ToColor = beforeBGR.ToColor then
      p^ := ({#}BGRALongWord(p^) and not ColorMask.ToColor) or afterBGR.ToColor;
    {$ELSE}//#
...
var p: {$IFDEF BDS}PBGRAPixel;{$ELSE}PBGRALongWord;{$ENDIF}//#
...
begin
  ...
  begin
    p := {$IFDEF BDS}Scanline[yb]+ABounds.Left;{$ELSE}PBGRALongWord(Scanline[yb]+ABounds.Left);{$ENDIF}
    for xb := xcount-1 downto 0 do
    begin
      {$IFDEF BDS}
      if {#}BGRALongWord(p^) and {#}BGRALongWord(colorMask) = {#}BGRALongWord(beforeBGR) then
        p^ := ({#}BGRALongWord(p^) and not {#}BGRALongWord(ColorMask)) or {#}BGRALongWord(afterBGR);
      {$ELSE}//#
      if p^ and colorMask = beforeBGR then
        p^ := (p^ and not ColorMask) or afterBGR;
      {$ENDIF}
...
var
  ...
  Visited: array of BGRALongWord;
  VisitedLineSize: integer;

  Stack:      array of integer;
  StackCount: integer;

  function CheckPixel(AX, AY: integer): boolean; //@{$ifdef inline}inline;{$endif}
  var
  ...
  var
    StartMask, EndMask: BGRALongWord;
  ...
  procedure Push(AX, AY: integer); //@{$ifdef inline}inline;{$endif}
  begin
  ...
  procedure Pop(var AX, AY: integer); //@{$ifdef inline}inline;{$endif}
  begin
    Dec(StackCount);
    AY := Stack[StackCount];
    Dec(StackCount);
...
var
  p: PBGRAPixel;
  n: integer;
  colormask: BGRALongWord;
begin
  if CanvasAlphaCorrection then
  begin
    p := FData;
    colormask := BGRALongWord(BGRA(255,255,255,0));
    for n := NbPixels - 1 downto 0 do
    begin
      if (BGRALongWord(p^) and colormask <> 0) and (p^.alpha = 0) then
...
begin
  ...
  begin
    FirstPoint(AffMatrixMult(AMatrix,PointF(ASourceBounds.Left-pointMargin,ASourceBounds.Top-pointMargin)));
    IncludePoint(AffMatrixMult(AMatrix,PointF(ASourceBounds.Right-1+pointMargin,ASourceBounds.Top-pointMargin)));
    IncludePoint(AffMatrixMult(AMatrix,PointF(ASourceBounds.Left-pointMargin,ASourceBounds.Bottom-1+pointMargin)));
    IncludePoint(AffMatrixMult(AMatrix,PointF(ASourceBounds.Right-1+pointMargin,ASourceBounds.Bottom-1+pointMargin)));
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not BGRAPixelEqual(pself^, {<>} pcomp^) then
        begin
          Result := False;
          exit;
        end;
...
begin
  ...
  begin
    if not BGRAPixelEqual(p^, {<>} comp) then
    begin
      Result := False;
      exit;
    end;
...
begin
  ...
  if n = 0 then
    exit;
  repeat
    p^   := ({$IFDEF BDS}BGRALongWord(p^){$ELSE}p^{$ENDIF} shr TBGRAPixel_RedShift and $FF) shl TBGRAPixel_AlphaShift;
    Inc(p);
    Dec(n);
  until n = 0;
  InvalidateBitmap;
...
begin
  ...
  if n = 0 then
    exit;
  repeat
    temp := ({$IFDEF BDS}BGRALongWord(p^){$ELSE}p^{$ENDIF} shr TBGRAPixel_AlphaShift) and $ff;
...
const tx = 8; ty = 8; //must be a power of 2
      xMask = tx*2-1;
var xcount,patY,w,n,patY1,patY2m1,patX,patX1: BGRANativeInt;
    pdest: PBGRAPixel;
    delta: BGRAPtrInt;
    actualRect: TRect;
{$IFDEF BDS}_BGRADWord : BGRADWord;{$ENDIF}//#
...
begin
  ...
  begin
    ...
    begin
      if not BGRAPixelEqual(p^, {<>} p2^) then
      begin
        if xb < minx then
          minx := xb;
        if yb < miny then
