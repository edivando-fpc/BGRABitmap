...
uses
  Classes, SysUtils, BGRATypes, Graphics, Types, BGRABitmapTypes, BGRAPhongTypes, BGRAText,
  BGRACustomTextFX, BGRAVectorize, BGRAGraphics;
...
begin
  ...
  begin
    ...
    begin
      ...
      metric.CapLine := metric.CapLine -toAdd;
  ...
  begin
    ...
    begin
      ...
      if yb < 2 then
      begin
        newCenter  := newCenter  +parts[yb].Height;
        prevCenter := prevCenter +(lines[yb]-fromy);
      end else
      if yb = 2 then
      begin
        newCenter  := newCenter  +(parts[yb].Height/2 );
        prevCenter := prevCenter +((lines[yb]-fromy)/2);
      end;
    end else
      parts[yb] := nil;
  end;
...
begin
  ...
  size.cx := size.cx +(2*overhang + ceil(SubOffsetX));
  size.cy := size.cy +(2 + ceil(SubOffsetY));
...
begin
  ...
  begin
    ...
    sizeX := sizeX -(sizeX mod FXAntialiasingLevel);

    sizeY := (sizeY + FXAntialiasingLevel-1);
    sizeY := sizeY -(sizeY mod FXAntialiasingLevel);

    if SubOffsetX <> 0 then
    begin
      sizeX := sizeX + ceil(SubOffsetX*FXAntialiasingLevel);
      iSubX := round(SubOffsetX*FXAntialiasingLevel);
    end;
    if SubOffsetY <> 0 then
    begin
      sizeY := sizeY +ceil(SubOffsetY*FXAntialiasingLevel);
  ...
  begin
    ...
    SizeX := SizeX -(SizeX mod GrainX);
  end;
  if GrainY > 0 then
  begin
    SizeY := SizeY+ (GrainY-1);
    SizeY := SizeY -(SizeY mod GrainY);
  end;
