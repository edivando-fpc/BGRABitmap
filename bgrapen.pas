...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      finalNb := finalNb +delta;
    end else delta := 0;
    if hasEnd then finalNb := finalNb +length(arrowEndData)+1;
  ...
  begin
    ...
    begin
      ...
      if (i=high(pts)-1) then
        pts[high(pts)] := pts[high(pts)] +(dir*hw);
...
var
  results: array of {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
  nbResults,nbTotalPts: integer;

  procedure AddWidePolyline(startIndex,endIndexP1: integer);
  var
    tempWidePolyline: {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
...
begin
  if AffMatrixEqual(FOriginalStrokeMatrix,{=}AValue) then Exit;
  ...
  if FStrokeZoom > 0 then
    FStrokeMatrix := AffMatrixMult(FStrokeMatrix, AffineMatrixScale(1/FStrokeZoom,1/FStrokeZoom));
...
begin
  ...
  if FStrokeMatrixIdentity then
    ...
    result := AffMatrixMult(FStrokeMatrix,ComputePolyline(AffMatrixMult(FStrokeMatrixInverse,APoints),AWidth*FStrokeZoom,c,AClosedCap));
end;
...
begin
  options := [];
  if Assigned(Arrow) and Arrow.IsStartDefined then options := options +[plNoStartCap];
  if Assigned(Arrow) and Arrow.IsEndDefined then options := options +[plNoEndCap];
  if not AClosedCap then options := options +[plRoundCapOpen];
  if FStrokeMatrixIdentity then
    ...
    result := AffMatrixMult(FStrokeMatrix,BGRAPen.ComputeWidePolylinePoints(AffMatrixMult(FStrokeMatrixInverse,APoints), AWidth*FStrokeZoom, APenColor, LineCap, JoinStyle, CustomPenStyle, options, MiterLimit, Arrow));
end;
...
begin
  ...
  if FStrokeMatrixIdentity then
    ...
    result := AffMatrixMult(FStrokeMatrix,BGRAPen.ComputeWidePolylinePoints(AffMatrixMult(FStrokeMatrixInverse,APoints), AWidth*FStrokeZoom, BGRAPixelTransparent, LineCap, JoinStyle, CustomPenStyle, options, MiterLimit, Arrow))
end;
...
begin
  if FStrokeMatrixIdentity then
    ...
    result := AffMatrixMult(FStrokeMatrix,BGRAPen.ComputeWidePolylinePoints(AffMatrixMult(FStrokeMatrixInverse,APoints), AWidth*FStrokeZoom, BGRAPixelTransparent, LineCap, JoinStyle, CustomPenStyle, [plCycle], MiterLimit));
end;
