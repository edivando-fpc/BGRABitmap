...
uses sysutils, classes, BGRATypes, {$IFNDEF FPC}Types, GraphType, BGRAGraphics,{$ENDIF}
     FPImgCmn, {$IFDEF FPC}PNGcomn, ZStream,{$ELSE}JPEG, ZLib,{$ENDIF} FPImage;
...
type
  ...
  EightLong = array[0..7] of BGRALongWord;
  TChunkCode = array[0..3] of {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF};

  TChunk = record
    acapacity, alength, CRC : BGRALongWord;
    ReadType : TChunkCode;
    data : PByteArray;
    aType : TChunkTypes;
  end;
...
const
  ...
  All1Bits : BGRALongWord = $FFFFFFFF;
...
type
  THeaderChunk = packed record
    Width, height : BGRALongWord;
    BitDepth, ColorType, Compression, Filter, Interlace : byte;
  end;

  TGetPixelFunc = function (x,y : BGRALongWord) : TColorData of object;
  ...
  TBGRAWriterPNG = class (TFPCustomImageWriter)
    private
      ...
      function GetColorPixel (x,y:BGRALongWord) : TColorData;
      function GetPalettePixel (x,y:BGRALongWord) : TColorData;
      function GetColPalPixel (x,y:BGRALongWord) : TColorData;
    ...
    protected
      ...
      function CurrentLine (x:BGRALongWord) : byte;
      function PrevSample (x:BGRALongWord): byte;
      function PreviousLine (x:BGRALongWord) : byte;
      function PrevLinePrevSample (x:BGRALongWord): byte;
      function  DoFilter (LineFilter:byte;index:BGRALongWord; b:byte) : byte; virtual;
      procedure SetChunkLength (aValue : BGRALongWord);
      ...
      function DetermineFilter ({%H-}Current, {%H-}Previous:PByteArray; {%H-}linelength:BGRALongWord):byte; virtual;
      ...
      property DatalineLength : BGRALongWord read FDatalineLength;
    public
...
procedure TBGRAWriterPNG.SetChunkLength(aValue : BGRALongWord);
begin
  with Fchunk do
    begin
    alength := aValue;
...
function TBGRAWriterPNG.CurrentLine(x:BGRALongWord):byte;
begin
  result := FCurrentLine^[x];
end;

function TBGRAWriterPNG.PrevSample (x:BGRALongWord): byte;
begin
  if x < byteWidth then
    result := 0
  else
...
function TBGRAWriterPNG.PreviousLine (x:BGRALongWord) : byte;
begin
  result := FPreviousline^[x];
end;

function TBGRAWriterPNG.PrevLinePrevSample (x:BGRALongWord): byte;
begin
  if x < byteWidth then
    result := 0
  else
...
function TBGRAWriterPNG.DoFilter(LineFilter:byte;index:BGRALongWord; b:byte) : byte;
var diff : byte;
  procedure FilterSub;
  begin
  ...
  var dl, dp, dlp : BGRAWord; // index for previous and distances for:
  ...
  begin
    ...
    p := PreviousLine(index);
    r := BGRANativeInt(l) + BGRANativeInt(p) - BGRANativeInt(lp);
    dl := abs (r - l);
    dlp := abs (r - lp);
    dp := abs (r - p);
    ...
    with AHeader do
      case colortype of
        0 : if FWordSized then
              begin
              FFmtColor := {$IFDEF OBJ}@{$ENDIF}ColorDataGrayW;
              FByteWidth := 2;
              //CFmt := cfGray16
              end
            else
              begin
              FFmtColor := {$IFDEF OBJ}@{$ENDIF}ColorDataGrayB;
              FByteWidth := 1;
              //CFmt := cfGray8;
              end;
        2 : if FWordSized then
              begin
              FFmtColor := {$IFDEF OBJ}@{$ENDIF}ColorDataColorW;
              FByteWidth := 6;
              //CFmt := cfBGR48
              end
            else
              begin
              FFmtColor := {$IFDEF OBJ}@{$ENDIF}ColorDataColorB;
              FByteWidth := 3;
              //CFmt := cfBGR24;
              end;
        4 : if FWordSized then
              begin
              FFmtColor := {$IFDEF OBJ}@{$ENDIF}ColorDataGrayAW;
              FByteWidth := 4;
              //CFmt := cfGrayA32
              end
            else
              begin
              FFmtColor := {$IFDEF OBJ}@{$ENDIF}ColorDataGrayAB;
              FByteWidth := 2;
              //CFmt := cfGrayA16;
              end;
        6 : if FWordSized then
              begin
              FFmtColor := {$IFDEF OBJ}@{$ENDIF}ColorDataColorAW;
              FByteWidth := 8;
              //CFmt := cfABGR64
              end
            else
              begin
              FFmtColor := {$IFDEF OBJ}@{$ENDIF}ColorDataColorAB;
              FByteWidth := 4;
              //CFmt := cfABGR32;
              end;
      end;
...
begin
  with AHeader do
    begin
    {$IFDEF ENDIAN_LITTLE}
    // problem: TheImage has integer width, PNG header BGRALongWord width.
    //          Integer Swap can give negative value
    Width := swap (BGRALongWord(TheImage.Width));
    height := swap (BGRALongWord(TheImage.Height));
    {$ELSE}
...
begin
  {$IFDEF FPC}{$PUSH}{$ENDIF}{$HINTS OFF}
  {$IFDEF BDS}
  result := hi(color.red) + (color.green and $FF00) + (hi(color.blue) shl 16) + (hi(color.alpha) shl 24);
  {$ELSE}
  with color do
...
{$IFDEF BDS}var _Int64, _2Int64 : BGRAInt64;{$ENDIF}
begin
  {$IFDEF FPC}{$PUSH}{$ENDIF}{$HINTS OFF}
  {$IFDEF BDS}
  move(color.alpha , _Int64, sizeof(word));
  _2Int64 := color.red + (color.green shl 16) + (color.blue shl 32) + (_Int64 shl 48);
  move(_2Int64 , result, sizeof(qword));
  {$ELSE}
  with color do
...
function TBGRAWriterPNG.GetColorPixel (x,y:BGRALongWord) : TColorData;
begin
  result := FFmtColor (TheImage[x,y]);
  //result := ConvertColorToData(TheImage.Colors[x,y],CFmt);
end;

function TBGRAWriterPNG.GetPalettePixel (x,y:BGRALongWord) : TColorData;
begin
  result := TheImage.Pixels[x,y];
end;

function TBGRAWriterPNG.GetColPalPixel (x,y:BGRALongWord) : TColorData;
begin
...
begin
  case Fheader.colortype of
    3 : if TheImage.UsePalette then
          begin
          result := {$IFDEF OBJ}@{$ENDIF}GetPalettePixel;
          end
        else
          begin
          result := {$IFDEF OBJ}@{$ENDIF}GetColPalPixel;
          end;
    else  begin
          result := {$IFDEF OBJ}@{$ENDIF}GetColorPixel;
          end
  end;
end;
...
begin
  ...
  case Fheader.colortype of
    3 : if TheImage.UsePalette then
          FGetPixel := GetPalettePixel
        else
          FGetPixel := GetColPalPixel;
    else
          FGetPixel := GetColorPixel;
  end;
  {$ENDIF}
end;
...
function TBGRAWriterPNG.DetermineFilter (Current, Previous:PByteArray; linelength:BGRALongWord) : byte;
begin
  result := 0;
end;
