...
uses
  Classes, SysUtils, BGRATypes,{$IFNDEF FPC}Types, GraphType,{$ENDIF} BGRAGraphics, BGRABitmapTypes, BGRAFillInfo, BGRASSE;
...
type
  ...
  protected
    ...
    FLightnesses: array of BGRAWord;
    procedure SetIntersectionValues(AInter: TIntersectionInfo; AInterX: single; AWinding,
      ANumSegment: integer; dy: single; AData: pointer); override;
  public
    constructor Create(const points: array of TPointF; const texCoords: array of TPointF); overload;
    constructor Create(const points: array of TPointF; const texCoords: array of TPointF; const lightnesses: array of BGRAWord); overload;
    function CreateSegmentData(numPt,nextPt: integer; x,y: single): pointer; override;
    function CreateIntersectionInfo: TIntersectionInfo; override;
  end;
...
procedure PolygonLinearTextureMappingAliasedWithLightness(bmp: TBGRACustomBitmap; const points: array of TPointF; texture: IBGRAScanner;
  const texCoords: array of TPointF; TextureInterpolation: Boolean; lightnesses: array of BGRAWord; NonZeroWinding: boolean); overload;
...
type
  ...
  public
    ...
    constructor Create(const points: array of TPointF; const pointsZ: array of single; const texCoords: array of TPointF; const lightnesses: array of BGRAWord); overload;
    function CreateSegmentData(numPt,nextPt: integer; x,y: single): pointer; override;
    function CreateIntersectionInfo: TIntersectionInfo; override;
  end;
...
procedure PolygonPerspectiveTextureMappingAliasedWithLightness(bmp: TBGRACustomBitmap; const points: array of TPointF; const pointsZ: array of single; texture: IBGRAScanner;
           const texCoords: array of TPointF; TextureInterpolation: Boolean; lightnesses: array of BGRAWord; NonZeroWinding: boolean; zbuffer: psingle = nil); overload;
...
begin
  ...
  TPerspectiveColorGradientIntersectionInfo(AInter).ColorDivZ := ColorFAdd(info^.ColorDivZ, ColorFMult(info^.ColorSlopesDivZ, dy));
end;
...
begin
  ...
  CurColorDivByZ := ColorFMult(FColors[numPt],CurInvZ);
  NextInvZ := 1/FPointsZ[nextPt];
  NextColorDivByZ := ColorFMult(FColors[nextPt],NextInvZ);

  info^.ColorSlopesDivZ := ColorFMult((ColorFSub(NextColorDivByZ, CurColorDivByZ)),InvTy);
  dy := y-FPoints[numPt].y;
  info^.ColorDivZ := ColorFAdd(CurColorDivByZ, ColorFMult(info^.ColorSlopesDivZ,dy));
...
begin
  ...
  TLinearColorGradientIntersectionInfo(AInter).color := ColorFAdd(info^.Color, ColorFMult(info^.ColorSlopes,dy));
end;
...
begin
  ...
  info^.ColorSlopes := ColorFMult((ColorFSub(FColors[nextPt], FColors[numPt])),(1/ty));
  dy := y-FPoints[numPt].y;
  info^.Color := ColorFAdd(FColors[numPt], ColorFMult(info^.ColorSlopes,dy));
  Result:= info;
end;
{$hints on}
...
var
  ...
  begin
    t := ((ix1+0.5)-x1)/(x2-x1);
    colorPos := ColorFAdd(c1, ColorFMult((ColorFSub(c2,c1)),t));
    colorStep := ColorFMult((ColorFSub(c2,c1)),(1/(x2-x1)));
    ...
    begin
      ...
      ec.alpha := round(colorPos[4]);
      DrawPixelInlineWithAlphaCheck(pdest, GammaCompression(ec));
      colorPos := ColorFAdd(colorPos, colorStep);
      inc(pdest);
    end;
  end;
...
constructor TPolygonLinearTextureMappingInfo.Create(
  ...
  const lightnesses: array of BGRAWord);
var
  i: Integer;
  lPoints: array of TPointF;
  nbP: integer;
...
var
  ...
  begin
    invDx := 1/(x2-x1);
    t := ((ix1+0.5)-x1)*InvDx;
    diff := ColorFSub(info2.ColorDivZ,info1.ColorDivZ);
    colorPos := ColorFAdd(info1.ColorDivZ, ColorFMult(diff,t));
    colorStep := ColorFMult(diff,InvDx);
...
begin
  ...
  {$ELSE}
  scanAtFunc := TBGRACustomBitmap(texture.GetInstance).ScanAt;
  scanAtIntegerFunc := TBGRACustomBitmap(texture.GetInstance).ScanAtInteger;
  {$ENDIF}
  inter := polyInfo.CreateIntersectionArray;
...
procedure PolygonLinearTextureMappingAliasedWithLightness(
  ...
  TextureInterpolation: Boolean; lightnesses: array of BGRAWord;
  NonZeroWinding: boolean);
var polyInfo: TPolygonLinearTextureMappingInfo;
begin
...
begin
  ...
  case ADrawMode of
  dmSetExceptTransparent: begin PixelProc := {$IFDEF OBJ}@{$ENDIF}dest.SetPixel; skipBorder:= BorderColor.alpha <> 255; end;  dmDrawWithTransparency: begin PixelProc := {$IFDEF OBJ}@{$ENDIF}dest.DrawPixel; skipBorder:= BorderColor.alpha = 0; end;
  dmXor: begin PixelProc := {$IFDEF OBJ}@{$ENDIF}dest.XorPixel; skipBorder:= BGRADWord(BorderColor) = 0; end;
  dmLinearBlend: begin PixelProc := {$IFDEF OBJ}@{$ENDIF}dest.FastBlendPixel; skipBorder:= BorderColor.alpha = 0; end;
  else
  begin PixelProc := {$IFDEF OBJ}@{$ENDIF}dest.SetPixel; skipBorder := false; end;
  end;
