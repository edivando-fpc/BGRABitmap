...
uses
  Classes, SysUtils, BGRATypes,{$IFNDEF FPC}Types, GraphType, BGRAGraphics,{$ENDIF} BGRABitmapTypes, BGRAColorInt,
...
begin
  ...
  FLookWhere := MatrizMult(MatrizMult(MatrizMult(inv, m), Matrix), FLookWhere);
  FMatrixComputed := false;
end;
...
begin
  ...
  for i := 0 to Object3DCount-1 do
    result := result +Object3D[i].TotalVertexCount;
end;
...
begin
  result := 0;
  for i := 0 to Object3DCount-1 do
    result := result +Object3D[i].FaceCount;
end;
...
begin
  result := 0;
  for i := 0 to Object3DCount-1 do
    result := result +Object3D[i].TotalNormalCount;
end;
...
var
  LFaces: {$IFDEF BDS}arrayofTBGRAFace3D{$ELSE}array of TBGRAFace3D{$ENDIF};
     ...
     begin
       ...
       case faceDesc.NormalsMode of
         ...
         lnFaceVertexMix:
             for j := 0 to VCount-1 do
             begin
               faceDesc.Normals3D[j] := faceDesc.Normals3D[j] +ViewNormal_128;
       ...
       if FRenderer.RenderFace(faceDesc, {$IFDEF OBJ}@{$ENDIF}ComputeCoordinate) then
...
begin
  ...
  m.OnTextureChanged := {$IFDEF OBJ}@{$ENDIF}OnMaterialTextureChanged;
  result := m;
  AddMaterial(result);
end;
