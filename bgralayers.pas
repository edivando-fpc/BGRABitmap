...
uses
  Classes, SysUtils, Types, BGRATypes, BGRABitmapTypes, {$IFNDEF FPC}GraphType, SyncObjs, {$ENDIF} BGRABitmap, BGRAGraphics,
  BGRAMemDirectory, BGRATransform, {$IFDEF OBJ}fgl{$ELSE}generics.collections{$ENDIF}, BGRALayerOriginal;
...
type
  ...
  TBGRALayerOriginalEntry = record
     ...
     class operator{$IFDEF OBJ}={$ELSE}Equal{$ENDIF}(const AEntry1,AEntry2: TBGRALayerOriginalEntry): boolean;
  end;


  TBGRALayerOriginalList = {$IFDEF OBJ}specialize TFPGList{$ELSE}TList{$ENDIF}<TBGRALayerOriginalEntry>;
  ...
  TBGRACustomLayeredBitmap = class(TGBRACustomGraphic)
  private
  ...
  function BGRALayerOriginalEntry(AGuid: TGuid): TBGRALayerOriginalEntry; overload;
  function BGRALayerOriginalEntry(AInstance: TBGRALayerCustomOriginal): TBGRALayerOriginalEntry; overload;
...
function Equal(const AGuid1, AGuid2: TGuid): boolean;
...
var
  NextLayerUniqueId: {$IFDEF FPC}cardinal;{$ELSE}Int64;{$ENDIF}
...
class operator TBGRALayerOriginalEntry.{$IFDEF OBJ}={$ELSE}Equal{$ENDIF}(const AEntry1,
  AEntry2: TBGRALayerOriginalEntry): boolean;
begin
  result := Equal(AEntry1.Guid, AEntry2.Guid);
end;
...
begin
  ...
  begin
    if Equal(FLayers[layer].OriginalGuid, {=} GUID_NULL) then exit(nil);
...
begin
  ...
  begin
    if AffMatrixEqual(FLayers[layer].OriginalMatrix, {=} AValue) then exit;
    FLayers[layer].OriginalMatrix := AValue;
    if not Equal(FLayers[layer].OriginalGuid, {<>} GUID_NULL) then
    begin
...
begin
  ...
  begin
    if Equal(FLayers[layer].OriginalGuid, {=} AValue) then exit;
    FLayers[layer].OriginalGuid := AValue;

    if not Equal(AValue, {<>} GUID_NULL) and (IndexOfOriginal(AValue) <> -1) then
    begin
      FLayers[layer].OriginalRenderStatus := orsNone;
      Unfreeze(layer);
    end;
...
begin
  ...
  for i := 0 to NbLayers-1 do
    if Equal(LayerOriginalGuid[i], {=} orig.Guid) then
...
begin
  ...
  if (result = nil) and not Equal(guid, {<>} GUID_NULL) then
  begin
    ...
    result.OnChange:= {$IFDEF OBJ}@{$ENDIF}OriginalChange;
  end;
end;
...
var
  newGuid: TGuid;
begin
  if Equal(AOriginal.Guid, {=} GUID_NULL) then
  begin
  ...
  AOriginal.OnChange:= {$IFDEF OBJ}@{$ENDIF}OriginalChange;
end;
...
begin
  ...
  guid := FOriginals[AIndex].Guid;
  for i := 0 to NbLayers-1 do
    if Equal(LayerOriginalGuid[i], {=} guid) then
    begin
...
var
  ...
  begin
    for i := 0 to nbFoundGuid-1 do
      if Equal(foundGuid[i], {=} AGuid) then exit(i);
    result := -1;
  end;
...
begin
  ...
  begin
    ...
    orig.Render(FLayers[layer].Source, AffMatrixMult(AffineMatrixTranslation(-rInter.Left,-rInter.Top),FLayers[layer].OriginalMatrix), ADraft);
...
begin
  ...
  begin
    ...
    if idxOrig <> -1 then useCount[idxOrig] := useCount[idxOrig] +1;
  end;
...
begin
  result := {$IFDEF FPC}InterLockedIncrement(NextLayerUniqueId);{$ELSE}TInterlocked.Increment(NextLayerUniqueId);{$ENDIF}
end;
...
begin
  ...
  for i := 0 to NbLayers-1 do
    if not Equal(LayerOriginalGuid[i], {<>} GUID_NULL) then
      LayerOriginalMatrix[i] := AffMatrixMult(AffMatrixMult(AffineMatrixTranslation(Width,0),AffineMatrixRotationDeg(90)),LayerOriginalMatrix[i])
    else
...
begin
  ...
  for i := 0 to NbLayers-1 do
    if not Equal(LayerOriginalGuid[i], {<>} GUID_NULL) then
      LayerOriginalMatrix[i] := AffMatrixMult(AffMatrixMult(AffineMatrixTranslation(0,Height),AffineMatrixRotationDeg(-90)),LayerOriginalMatrix[i])
    else
...
begin
  ...
  begin
    ...
    FLayers[i].OriginalMatrix := AffMatrixMult(AffMatrixMult(AffMatrixMult(AffineMatrixTranslation(+Width/2,0),AffineMatrixScale(-1,1)),AffineMatrixTranslation(-Width/2,0)),FLayers[i].OriginalMatrix);
  end;
end;
...
begin
  ...
  begin
    ...
    FLayers[i].OriginalMatrix := AffMatrixMult(AffMatrixMult(AffMatrixMult(AffineMatrixTranslation(0,+Height/2),AffineMatrixScale(1,-1)),AffineMatrixTranslation(0,-Height/2)),FLayers[i].OriginalMatrix);
  end;
end;
...
begin
  ...
  for i := 0 to NbLayers-1 do
  if not Equal(FLayers[i].OriginalGuid, {<>} GUID_NULL) then
    LayerOriginalMatrix[i] := AffMatrixMult(AffineMatrixScale(AWidth/prevWidth,AHeight/prevHeight),LayerOriginalMatrix[i])
  else
  begin
...
begin
  ...
  begin
    ...
    FOriginalEditor.Matrix := AffMatrixMult(AMatrix,LayerOriginalMatrix[ALayerIndex]);
...
begin
  for i := 0 to OriginalCount-1 do
    if Equal(FOriginals[i].Guid, {=} AGuid) then
    begin
      result := i;
      exit;
    end;
...
function TBGRACustomLayeredBitmap.ToString: {$IFDEF FPC}ansistring;{$ELSE}string;{$ENDIF}
var
  i: integer;
begin
...
procedure TBGRACustomLayeredBitmap.Freeze(firstLayer, lastLayer: integer);
  ...
  begin
    ...
    for i := first to last do
      if LayerVisible[i] and (LayerOpacity[i] > 0) then nbVisible := nbVisible +1;
