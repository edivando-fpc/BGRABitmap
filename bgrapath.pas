...
begin
  ...
  begin
    ...
    while Path.GoToPreviousElement(pos) do
      FArcPos := FArcPos +Path.GetElementLength(pos, FAcceptedDeviation);
  end;
  result := FArcPos;
end;
...
begin
  ...
  result := 0;
  repeat
    result := result +GetElementLength(pos, AAcceptedDeviation);
...
var sub: array of ArrayOfTPointF;
    temp: ArrayOfTPointF;
    nbSub,nbPts,curPt,curSub: BGRANativeInt;
    startPos,pos: BGRAPtrInt;
...
begin
  ...
  if not IsAffineMatrixIdentity(AMatrix) then
    result := AffMatrixMult(AMatrix,result);
end;
...
var empty: boolean;
    ...
    elemType: TBGRAPathElementType;
    elem: pointer;
    temp: {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
    i: integer;
...
begin
  ...
  data.Color := AColor;
  data.Width := AWidth;
  InternalDraw({$IFDEF OBJ}@{$ENDIF}BitmapDrawSubPathProc, AMatrix, AAcceptedDeviation, @data);
end;
...
var
  ...
  lastPosF: TPointF;
  implicitCommand: {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF};
  ...
  procedure addCommand(command: {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF}; parameters: string);
  begin
    if result <> '' then result := result +' '; //optional whitespace
    if command <> implicitCommand then result := result +command;
    result := result +trim(parameters);
...
var elemType: TBGRAPathElementType;
    ...
    Pos: BGRAPtrInt;
    p1: TPointF;
    pts: {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
    i: integer;
begin
...
begin
  FLastCoord := ACoord;
  FLastTransformedCoord := AffMatrixMult(FMatrix,ACoord);
...
begin
  ...
  begin
    ControlPoint1 := tcp1;
    ControlPoint2 := AffMatrixMult(FMatrix,cp2);
...
function TBGRAPath.GetPolygonalApprox(APos: BGRAPtrInt; AAcceptedDeviation: single; AIncludeFirstPoint: boolean): ArrayOfTPointF;
...
var
  ...
  procedure OutputSub(subPathStartPos, subPathEndPos: BGRAPtrInt);
  var
    sub: array of ArrayOfTPointF;
    temp: ArrayOfTPointF;
    startPos,pos,nbPts,curPt,curSub: BGRAPtrInt;
    elemType: TBGRAPathElementType;
    elem: pointer;
  begin
    pos := subPathStartPos;
    ...
    if not IsAffineMatrixIdentity(AMatrix) then
      temp := AffMatrixMult(AMatrix,temp);
...
var
  subPathStartPos: BGRAPtrInt;
  prevPos,pos: BGRAPtrInt;
  elemType: TBGRAPathElementType;
  elem: pointer;
begin
  AAcceptedDeviation := CorrectAcceptedDeviation(AAcceptedDeviation, AMatrix);
...
var p: integer;
  ...
  begin
    result.x := parseFloat;
    result.y := parseFloat;
    if relative and not isEmptyPointF(lastCoord) then result := result +lastCoord;
    if isEmptyPointF(lastCoord) then startCoord := result;
  end;

var
  command,implicitCommand: {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF};
  relative: boolean;
  c1,c2,p1: TPointF;
  a: TArcDef;
  largeArc: boolean;
begin
  ...
  begin
    command := {$IFDEF FPC}Char{$ELSE}AnsiChar{$ENDIF}(AValue[p]);
    ...
    'H': begin
        ...
        begin
          p1 := lastCoord;
          if relative then p1.x := p1.x +parseFloat
    ...
    'V': begin
        ...
        begin
          p1 := lastCoord;
          if relative then p1.y := p1.y +parseFloat
...
begin
  OnModify;
  count := count +4; //avoid memory error
...
function TBGRAPath.AllocateElement(AElementType: TBGRAPathElementType;
  AExtraBytes: BGRAPtrInt): Pointer;
var t: BGRAPtrInt;
begin
...
function TBGRAPath.GoToNextElement(var APos: BGRAPtrInt): boolean;
var newPos: BGRAPtrInt;
  p: PSplineElement;
  elemType: TBGRAPathElementType;
begin
  ...
  begin
    ...
    begin
      ...
      newPos := newPos +(p^.NbControlPoints * sizeof(TPointF)); //extra
    end;
...
function TBGRAPath.GoToPreviousElement(var APos: BGRAPtrInt): boolean;
...
function TBGRAPath.PeekNextElement(APos: BGRAPtrInt): TBGRAPathElementType;
begin
...
function TBGRAPath.GetElementStartCoord(APos: BGRAPtrInt): TPointF;
var
  elemType: TBGRAPathElementType;
  elem: pointer;
begin
...
function TBGRAPath.GetElementEndCoord(APos: BGRAPtrInt): TPointF;
var elemType: TBGRAPathElementType;
  elem: pointer;
begin
  GetElementAt(APos, elemType, elem);
...
function TBGRAPath.GetElementLength(APos: BGRAPtrInt; AAcceptedDeviation: single): single;
var elemType: TBGRAPathElementType;
  elem: pointer;
  pts: {$IFDEF BDS}arrayofTPointF{$ELSE}array of TPointF{$ENDIF};
begin
  ...
  peArc: begin
      ...
      result := result +PolylineLen(ComputeArc(PArcElement(elem)^, 0.1/AAcceptedDeviation));
    end;
...
procedure TBGRAPath.GetElementAt(APos: BGRAPtrInt; out
...
begin
  OnMatrixChange;
  FMatrix := AffMatrixMult(FMatrix, AffineMatrixTranslation(x,y));
end;
...
begin
  OnMatrixChange;
  FMatrix := AffMatrixMult(FMatrix, AffineMatrixRotationRad(-angleRadCW));
  FAngleRadCW := FAngleRadCW +angleRadCW;
end;
...
begin
  OnMatrixChange;
  FMatrix := AffMatrixMult(FMatrix, AffineMatrixScale(factor,factor));
...
begin
  if LastCoordDefined then
    QuadraticCurveFromTransformed(AffMatrixMult(FMatrix,cp), pt) else
  begin
    lineTo(pt);
    FExpectedTransformedControlPoint := AffMatrixMult(FMatrix,(pt+(pt-cp)));
  end;
end;
...
begin
  ...
  BezierCurveFromTransformed(AffMatrixMult(FMatrix,cp1), cp2, pt);
end;
...
begin
  ...
  begin
    ...
    transformedArc.center := AffMatrixMult(FMatrix,arcDef.center);
...
{$IFDEF FPC}
function TBGRAPath.QueryInterface({$IFDEF FPC_HAS_CONSTREF}constref{$ELSE}const{$ENDIF} IID: TGUID; out Obj): HResult; {$IF (not defined(WINDOWS)) AND (not defined(FPC20501))}cdecl{$ELSE}stdcall{$IFEND};
{$ELSE}
function TBGRAPath.QueryInterface(const IID: TGUID; out Obj): HResult; stdcall;
{$ENDIF}
begin
  if GetInterface(iid, obj) then
    Result := S_OK
  else
    Result := BGRALongInt(E_NOINTERFACE);
end;

{ There is no automatic reference counting, but it is compulsory to define these functions }
{$IFDEF FPC}
function TBGRAPath._AddRef: Integer; {$IF (not defined(WINDOWS)) AND (not defined(FPC20501))}cdecl{$ELSE}stdcall{$IFEND};
{$ELSE}
function TBGRAPath._AddRef: Integer; stdcall;
{$ENDIF}
begin
  result := 0;
end;
...
{$IFDEF FPC}
function TBGRAPath._Release: Integer; {$IF (not defined(WINDOWS)) AND (not defined(FPC20501))}cdecl{$ELSE}stdcall{$IFEND};
{$ELSE}
function TBGRAPath._Release: Integer; stdcall;
{$ENDIF}
begin
  result := 0;
end;
