...
uses
  Classes, SysUtils, BGRATypes,{$IFNDEF FPC}Types, GraphType,{$ENDIF} BGRAFilterType, BGRAPalette, BGRABitmapTypes;

type
  TOutputPixelProc = procedure(X,Y: BGRANativeInt; AColorIndex: BGRANativeInt; AColor: TBGRAPixel) of object;
  ...
  protected
    ...
    FCurrentOutputY: BGRANativeInt;
    FOutputPixel : TOutputPixelProc;
    FDrawMode: TDrawMode;
    procedure OutputPixel(X,Y: BGRANativeInt; {%H-}AColorIndex: BGRANativeInt; AColor: TBGRAPixel); virtual;
  ...
  protected
    ...
    FPalette: TBGRACustomApproxPalette;
    FIgnoreAlpha: boolean;
    FTransparentColorIndex: BGRANativeInt;

    //following variables are used during dithering
    FCurrentScanlineSize: BGRAPtrInt;
    FCurrentData: PByte;
    FCurrentOutputY: BGRANativeInt;
    FCurrentOutputScanline: PByte;
    FCurrentBitOrderMask: BGRANativeInt;
    FCurrentMaxY: BGRANativeInt;
    ...
    procedure OutputPixelSubByte(X,Y: BGRANativeInt; AColorIndex: BGRANativeInt; {%H-}AColor: TBGRAPixel); virtual;
    procedure OutputPixelFullByte(X,Y: BGRANativeInt; AColorIndex: BGRANativeInt; {%H-}AColor: TBGRAPixel); virtual;
    function GetScanline(Y: BGRANativeInt): Pointer; virtual;
  ...
  public
    ...
    function DitherImage(AAlgorithm: TDitheringAlgorithm; AImage: TBGRACustomBitmap; AScanlineSize: BGRAPtrInt): Pointer; overload;
    ...
    procedure DitherImageTo(AAlgorithm: TDitheringAlgorithm; AImage: TBGRACustomBitmap; AData: Pointer; AScanlineSize: BGRAPtrInt); overload;
    function ComputeMinimumScanlineSize(AWidthInPixels: integer): BGRAPtrInt;
    function AllocateSpaceForIndexedData(AImage: TBGRACustomBitmap; AScanlineSize: BGRAPtrInt): pointer;
...
function AbsRGBADiff(const c1, c2: TExpandedPixel): BGRANativeInt;
begin
  result := abs(c1.alpha-c2.alpha);
  result := result +abs(c1.red-c2.red);
  result := result +abs(c1.green-c2.green);
  result := result +abs(c1.blue-c2.blue);
end;
...
procedure TDitheringToIndexedImage.OutputPixelSubByte(X, Y: BGRANativeInt;
  AColorIndex: BGRANativeInt; AColor: TBGRAPixel);
var p: PByte;
begin
...
procedure TDitheringToIndexedImage.OutputPixelFullByte(X, Y: BGRANativeInt;
  AColorIndex: BGRANativeInt; AColor: TBGRAPixel);
begin
  if y <> FCurrentOutputY then
  begin
  ...
  case FBitsPerPixel of
    ...
    16: (PBGRAWord(FCurrentOutputScanline)+x)^ := AColorIndex;
    32: (PBGRADWord(FCurrentOutputScanline)+x)^ := AColorIndex;
  end;
end;

function TDitheringToIndexedImage.GetScanline(Y: BGRANativeInt): Pointer;
begin
...
function TDitheringToIndexedImage.ComputeMinimumScanlineSize(
  AWidthInPixels: integer): BGRAPtrInt;
begin
...
function TDitheringToIndexedImage.AllocateSpaceForIndexedData(AImage: TBGRACustomBitmap;
  AScanlineSize: BGRAPtrInt): pointer;
var size: integer;
begin
...
function TDitheringToIndexedImage.DitherImage(AAlgorithm: TDitheringAlgorithm;
  AImage: TBGRACustomBitmap; AScanlineSize: BGRAPtrInt): Pointer;
begin
...
procedure TDitheringToIndexedImage.DitherImageTo(AAlgorithm: TDitheringAlgorithm;
  AImage: TBGRACustomBitmap; AData: Pointer; AScanlineSize: BGRAPtrInt);
...
begin
  ...
  try
    ...
    if BitsPerPixel >= 8 then
      ditherTask.OnOutputPixel := {$IFDEF OBJ}@{$ENDIF}OutputPixelFullByte
    else
    begin
      ditherTask.OnOutputPixel:= {$IFDEF OBJ}@{$ENDIF}OutputPixelSubByte;
...
procedure TDitheringTask.OutputPixel(X, Y: BGRANativeInt; AColorIndex: BGRANativeInt;
  AColor: TBGRAPixel);
begin
  if Y <> FCurrentOutputY then
  begin
...
begin
  ...
  FCurrentOutputY := -1;
  FCurrentOutputScanline:= nil;
  OnOutputPixel:= {$IFDEF OBJ}@{$ENDIF}OutputPixel;
...
var
  ...
  function Div16(AValue: BGRANativeInt): BGRANativeInt; inline;
  begin
    if AValue < 0 then
      result := -((-AValue) shr 4)
    else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            curPix.alpha := alpha+BGRANativeInt(orig.alpha shl ErrorPrecisionShift);
            curPix.red := red+BGRANativeInt(orig.red shl ErrorPrecisionShift);
            curPix.green := green+BGRANativeInt(orig.green shl ErrorPrecisionShift);
            curPix.blue := blue+BGRANativeInt(orig.blue shl ErrorPrecisionShift);
